...
trigger: frontend design, 前端设计, UI设计, 界面设计, 组件设计, 设计系统
description: 生产级前端界面设计
category: frontend
version: 2.1.0
author: hermes-ai
...
> hermes-frontend-design 是一个专业的 生产级前端界面设计，帮助用户快速完成相关任务并输出标准化交付物。
...
```
frontend design, 前端设计, UI设计, 界面设计, 组件设计, 设计系统
```
...
hermes-frontend-design 属于 **开发运维** 域（development and DevOps for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 hermes-frontend-design 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(hermes-frontend-design)
    B --> C[分析结果]
    B --> D[解决方案]
```
