---
name: hermes-project-session-manager
trigger: project session, 项目管理, 会话管理, 上下文切换, 多项目, 状态保持
description: 跨会话项目上下文管理
...
# hermes-project-session-manager

> hermes-project-session-manager 是一个专业的 跨会话项目上下文管理，帮助用户快速完成相关任务并输出标准化交付物。
...
```
project session, 项目管理, 会话管理, 上下文切换, 多项目, 状态保持
```
...
hermes-project-session-manager 属于 **开发运维** 域（development and DevOps for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 hermes-project-session-manager 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(hermes-project-session-manager)
    B --> C[分析结果]
    B --> D[解决方案]
```
