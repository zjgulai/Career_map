...
export const requireAuth: MiddlewareHandler = async (c, next) => {
  const deps = c.get('deps')
  const [REDACTED]'Authorization')?.replace('Bearer ', '')
...
const createMockDeps = (overrides: Partial<UserDeps> = {}): UserDeps => ({
  ...
  hasher: {
    hash: vi.fn([REDACTED] TE.right(`hashed_${password}`)),
...
describe('Validation Properties', () => {
  ...
  it('passwords meeting requirements should pass', () => {
    ...
    fc.assert(
      fc.property(validPassword, [REDACTED] {
