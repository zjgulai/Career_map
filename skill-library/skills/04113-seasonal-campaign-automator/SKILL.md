...
title: "季节性活动自动化"
description: "- Orchestrate and automate high-velocity seasonal marketing events (BFCM, Holidays, Prime Day). Implement discount scheduling, tiered access, and cross-channel coordination to maximize seasonal revenue peaks. 触发词：季节性活动、大促自动化、BFCM、节日营销、折扣排期、旺季活动策划。"
...
enabled: "true"
user-invocable: true
workflow: "配置自动折扣与排期；设置分层准入与 VIP 逻辑（VIP提前/候补/公开）；按预热/预告/高峰/恢复时间线执行；运行性能护栏（库存/负载/毛利保护）"
input_contract: 大促时间与大促目标（可选：VIP 分层、折扣深度）
output_contract: 折扣排期+分层准入+性能护栏方案（对话，实时）
example: 说「帮我编排 BFCM 大促」→ 得到预热到恢复的时间线、自动折扣与护库存方案。
...
## 触发词
- 季节性活动自动化、seasonal-campaign-automator、BFCM、大促等旺季营销编排自动化 等表述时使用。

## 何时使用
- BFCM、大促等旺季营销编排自动化。

## 何时不用
- 大促效果分析走 performance-tracking；营销活动策划走 campaign-planning；邮件序列走 email-automation-flow-builder
