...
trigger: create, custom, course, materials
description: Source: https://claude.com/resources/use-cases/create-custom-course-materials
...
> Create custom course materials 是一个专业的 Source: https://claude.com/resources/use-cases/create-custom-course-materials，帮助用户快速完成相关任务并输出标准化交付物。
...
Create custom course materials 属于 **通用** 域（general-purpose for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 Create custom course materials 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(Create custom course materials)
    B --> C[hermes-team]
    B --> D[hermes-ultraqa]
```
