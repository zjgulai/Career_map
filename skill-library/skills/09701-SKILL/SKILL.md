---
name: SKILL
trigger: skill, channel, operate/aplus, content,
...
> SKILL 是一个专业的 在 Listing 文案合规通过后，生成亚马逊 A+（或等价富媒体）模块草稿：技术参数、场景、安全认证、用户故事等模块占位与文案；不自动上传。，帮助用户快速完成相关任务并输出标准化交付物。
...
```
skill, channel, operate/aplus, content,
```
