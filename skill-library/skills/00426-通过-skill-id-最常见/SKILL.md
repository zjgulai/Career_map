...
- 不要打包隐藏文件 / 目录、可执行文件、密钥、超大文件、重复大小写路径或 symlink。
- 允许的资源类型以文档、配置、图片、表格和安全文本为主；如果 CLI 返回 `SKILLHUB_ZIP_VALIDATION_FAILED`，按 `error.message` 指向的文件 / 字段修包后重新打 zip，不要原样重试。
