...
description_zh: "用于设计、重设计、评审、优化前端界面。覆盖网站、落地页、仪表盘、产品 UI、组件、表单等。处理 UX 评审、视觉层级、信息架构、可访问性、性能、响应式、主题、反模式、排版、间距、布局、色彩、动效、微交互、UX 文案等。"
...
### Browser Automation (live command)
When the `live` command is active and browser automation MCP is connected:
- Use browser automation to launch and maintain a live preview session
- Enable HMR (Hot Module Replacement) for real-time code-to-browser feedback
- If browser automation is not connected, fall back to generating static HTML files and instruct the user to open them manually

### AI Image Generation (codex / craft command)
When generating placeholder images or visual assets during `craft`, `codex`, or `shape`:
- If AI image generation MCP is connected, use it to generate contextual placeholder images
- If not connected, use CSS gradients, geometric SVG patterns, or clearly labeled placeholder blocks

---

## Pitfalls / 质量红线

- **Side-stripe borders (border-left/right > 1px as accent) 永远不是有意为之** — `border-left` or `border-right` greater than 1px as a colored accent on cards, list items, callouts, or alerts. Never intentional. Rewrite with full borders, background tints, leading numbers/icons, or nothing.
- **Gradient text (background-clip: text + gradient) 纯装饰** — `background-clip: text` combined with a gradient background. Decorative, never meaningful. Use a single solid color. Emphasis via weight or size.
- **Glassmorphism 作为默认风格 = AI 审美** — Blurs and glass cards used decoratively. Rare and purposeful, or nothing. Glassmorphism as default is the saturated AI aesthetic of 2026.
- **Hero-metric template（大数字+小标签+渐变强调）= SaaS 套路** — Big number, small label, supporting stats, gradient accent. This is the SaaS cliché that instantly signals AI-generated content.
- **相同大小的卡片网格无限重复 = 懒设计** — Same-sized cards with icon + heading + text, repeated endlessly. Cards are the lazy answer. Use them only when they're truly the best affordance. Nested cards are always wrong.
- **32px+ border-radius 在卡片/section 上 = 过度圆角** — Cards top out at 12–16px; full-pill is fine for tags/buttons. Picking 24/28/32/40px on a card is the codex tell; no brand wants "insanely rounded".
- **"X theater" 类文案 = AI slop** — "Productivity theater", "engagement theater", "growth theater": instant AI slop. Choose a specific noun, not a meta-criticism phrase.
