---
name: hermes-ultrawork
trigger: ultrawork, ulw, 高吞吐量, 并行执行, 批量处理, 大规模任务
description: 高吞吐量并行执行
category: ai
version: 2.1.0
author: hermes-ai
...
> hermes-ultrawork 是一个专业的 高吞吐量并行执行，帮助用户快速完成相关任务并输出标准化交付物。
...
```
ultrawork, ulw, 高吞吐量, 并行执行, 批量处理, 大规模任务
```
...
hermes-ultrawork 属于 **开发运维** 域（development and DevOps for Momcozy），可与其他 skill 形成工作流链路：

- **error-handler**: 当本skill输出需要进一步处理时调用
- **hermes-schedule**: 当本skill输出需要进一步处理时调用
- **hermes-remember**: 当本skill输出需要进一步处理时调用

## 一句话调用

```
使用 hermes-ultrawork 帮我下一步行动或相关skill推荐...
```
