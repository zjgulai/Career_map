...
## Qoder Plugin Runtime 检查与安装

每个会话首次执行 `lark-cli` 前，先运行 `lark-cli-runtime-check`：

1. 检查成功时直接使用 `lark-cli`，不要重复安装。
2. 检查失败时告知用户需要安装公开版 Lark CLI，并说明安装需要 Node.js/npm 且会修改用户目录。
3. 取得用户明确同意后运行 `lark-cli-install`。该入口只安装 CLI Runtime，不会重复安装或覆盖本 Plugin 提供的 Skills。
4. 安装完成后再次运行 `lark-cli-runtime-check`，然后执行 `lark-cli --version` 验证。验证失败时报告安装器原始错误，不要绕过 `lark-cli` 改用 curl 或直接调用飞书 OpenAPI。

安装 Runtime 会修改用户环境，未经用户同意不得自动运行 `lark-cli-install`。宿主预置的 Runtime 与用户安装的公开版 Runtime 都通过同一个 `lark-cli` 命令入口使用。
