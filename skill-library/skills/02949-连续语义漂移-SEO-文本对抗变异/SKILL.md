...
> **本节的完整实现在同目录的 `references/implementation.py`（298 行）。**下面 60 行是它的**开头**，源站发布时就截在这里。
...
> 卡页另声明「代码块数量：1」并记录位置 `paper2skills-code/data_collection/continuous_nlp_seo_morphing` —— **该路径仍不在本包内**；本包的 `references/implementation.py` 取自语料 vault 的卡本身（`paper2skills-vault/22-数据采集工程/Skill-Continuous-NLP-SEO-Morphing.md`），已与卡面节选核对，不依赖上述路径。
