...
## Preflight permissions before the first burst

Run `macos-harness doctor` once before the first native-app work. If
`accessibility`, `screen_recording`, or `post_events` is false, stop and report it
to the user instead of calling primitives: every TCC-gated verb (`see`, `click`,
`key`, `type`, `ax`) fails with exit 1 and no round trip can repair it. `apps`,
`script`, and plain Python still work.

These grants are bound to the host app's code signature, so replacing or
rebuilding that app silently invalidates them: a harness that worked yesterday can
be dead today. Re-grant in System Settings, then relaunch the host app — Screen
Recording keeps denying until that relaunch, while Accessibility applies at once.
