...
trigger: taxonomy, normalizer
description: 术语标准化器，统一多来源数据中的术语表达。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> Taxonomy Normalizer 是一个专业的 术语标准化器，统一多来源数据中的术语表达。，帮助用户快速完成相关任务并输出标准化交付物。
...
Taxonomy Normalizer 属于 **通用** 域（general-purpose for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 Taxonomy Normalizer 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(Taxonomy Normalizer)
    B --> C[分析结果]
    B --> D[解决方案]
```
