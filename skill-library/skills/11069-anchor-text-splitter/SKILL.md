---
name: "anchor-text-splitter"
title: "锚点文本分割器"
description: "当用户需要基于锚点标记精确分割文本时使用，支持 OCR 容错与模糊匹配定位。触发词：锚点文本分割器、锚点分割、锚点定位分割、标记点切分、精确切分、OCR容错、模糊匹配分割。何时不用：简单固定长度切分、文档无明确标记点、需要语义理解的分割（使用semantic-doc-chunker）、锚点可能多次出现需消歧、知识单元分组聚类（使用semantic-bucketer）、文本密度评估（使用semantic-density-analyzer）、知识相似度检测（使用knowledge-similarity-analyzer）、网页采集字段设计（使用web-scraping-plan-designer）。"
user_summary: "按标记点精准切分文本：识别容错、模糊匹配都能处理。要按固定标记切长文本时，找它。"
enabled: "true"
...
|-----------|-------------|---------------|---------|
| semantic-doc-chunker | 基于用户指定锚点精确切分 | 基于语义边界自动分块 | 用户提供锚点→本 Skill；无锚点、要求语义完整性→semantic-doc-chunker |
| semantic-bucketer | 按锚点标记分割文本 | 按标签重叠度分组知识单元 | 输入为长文本+锚点→本 Skill；输入为知识单元+标签→semantic-bucketer |
| semantic-density-analyzer | 定位并分割文本 | 评估文本知识密度 | 要求分割→本 Skill；要求评估密度/热力图→semantic-density-analyzer |
| knowledge-similarity-analyzer | 分割文本为块 | 计算知识单元相似度 | 要求分割→本 Skill；要求对比/检测重复→knowledge-similarity-analyzer |
| web-scraping-plan-designer | 分割已有文本 | 设计网页数据采集方案 | 已有文本+锚点→本 Skill；需要设计采集字段/PRD→web-scraping-plan-designer |
