---
name: api-designer
trigger: api design, API设计, RESTful, OpenAPI, 接口规范, 端点设计
description: RESTful API设计专家
category: dev
version: 2.1.0
author: hermes-ai
...
> api-designer 是一个专业的 RESTful API设计专家，帮助用户快速完成相关任务并输出标准化交付物。
...
```
api design, API设计, RESTful, OpenAPI, 接口规范, 端点设计
```
...
api-designer 属于 **开发运维** 域（development and DevOps for Momcozy），可与其他 skill 形成工作流链路：

- **test-strategist**: 当本skill输出需要进一步处理时调用
- **security-guard**: 当本skill输出需要进一步处理时调用
- **docker-master**: 当本skill输出需要进一步处理时调用

## 一句话调用

```
使用 api-designer 帮我下一步行动或相关skill推荐...
```
