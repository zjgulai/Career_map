---
name: SCM Inventory Forecaster - 库存预测专家
trigger: scm, inventory, forecaster, , , 库存预测专家
description: 基于历史销量趋势、季节性波动进行智能库存预测，提供安全库存计算、补货决策支持和大促备货规划。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> SCM Inventory Forecaster - 库存预测专家 是一个专业的 基于历史销量趋势、季节性波动进行智能库存预测，提供安全库存计算、补货决策支持和大促备货规划。，帮助用户快速完成相关任务并输出标准化交付物。
...
SCM Inventory Forecaster - 库存预测专家 属于 **供应链** 域（supply chain management for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 SCM Inventory Forecaster - 库存预测专家 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(SCM Inventory Forecaster - 库存预测专家)
    B --> C[分析结果]
    B --> D[解决方案]
```
