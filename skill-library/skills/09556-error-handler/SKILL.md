---
name: error-handler
trigger: error handling, 错误处理, 异常处理, 容错设计, 降级策略, 熔断
description: 健壮错误处理策略
category: dev
version: 2.1.0
author: hermes-ai
...
> error-handler 是一个专业的 健壮错误处理策略，帮助用户快速完成相关任务并输出标准化交付物。
...
```
error handling, 错误处理, 异常处理, 容错设计, 降级策略, 熔断
```
...
error-handler 属于 **开发运维** 域（development and DevOps for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 error-handler 帮我下一步行动或相关skill推荐...
```
