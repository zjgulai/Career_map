...
name: "kami"
title: "纸感排版系统（Kami）"
user_summary: "把一份内容排成有纸感的正式文档：简历、一页纸、白皮书、作品集、研报、幻灯片、落地页"
user_try: "帮我做一份中文一页纸，输出 PDF"
description: "用 Kami 模板产出简历、一页纸、白皮书、长文、信件、作品集、个股研报、更新日志与幻灯片（HTML/PDF/PPTX/Marp），也做可部署的多语言落地页。触发词：做 PDF、排版、简历、一页纸、白皮书、作品集、研报、更新日志、一套幻灯片、落地页、Kami、kami。何时不用：要印刷级通用 PDF 版面用 minimax-pdf；单文件网页翻页 PPT 用 guizang-ppt-skill；可交互图表与整页 HTML 报告用 lieflat-charts；审计或改造已有产品站点不适用。安全边界：不虚构数据与素材，缺料标 [DATA NEEDED]；中文正文字体仓耳今楷02 商用需另行授权。"
enabled: "true"
user-invocable: true
metadata:
  source: "third-party"
  ...
  sha256: "bd13d70a93c8cdee876fbfb30f49d790a366594a228f5559c883a0acce1f3310"
...
## Step −1 · Runtime preconditions (DSH install)

This copy is installed as a user-level DSH skill at `~/.dsh/skills/kami/`. Two host adaptations apply and they override the upstream README:

- **The Python interpreter is the managed skills venv, not the system `python3`.** Every `python3 scripts/…` command written in this file means:

  ```bash
  PY="$HOME/.dsh/skills-runtime/.venv/bin/python"   # Python 3.12; weasyprint + pypdf live here
  "$PY" scripts/build.py --doctor
  ```

  The system `python3` (3.14) does **not** carry Kami's render dependencies — using it fails with `ModuleNotFoundError: weasyprint`. `bash scripts/…` helpers (fonts, MathJax) run on the normal shell and are unaffected.
- **The update check is disabled.** Do not run `scripts/check-update.sh`: this install is pinned to upstream commit `6b39d045` (v1.15.0) and makes no in-task network calls. Updating means re-running the install procedure, not a check at task start.
...
A change to `SKILL.md`, templates, scripts, or references reaches upstream installs on the next push to `main`. **This copy is frozen at commit `6b39d045`** (upstream v1.15.0) under `~/.dsh/skills/kami/`; nothing here updates itself, so name a local edit as "local only, unpinned from upstream" rather than calling it shipped.
