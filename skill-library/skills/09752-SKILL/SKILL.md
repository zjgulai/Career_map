---
name: SKILL
trigger: skill, channel, operate/market, complia
...
> SKILL 是一个专业的 当输入「目标市场 + 产品型号/品类（如吸奶器）」需要一份可执行的合规检查清单（北美或欧洲轨）时调用；上架、改 Listing、进 KA 或新开站点前必用。，帮助用户快速完成相关任务并输出标准化交付物。
...
```
skill, channel, operate/market, complia
```
