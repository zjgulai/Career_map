...
```bash
# start/end 时间范围（ISO 8601 / YYYY-MM-DD / Unix 秒），均可选；默认当天
# user-id 目标用户 open_id，可重复或用逗号分隔；默认当前登录用户
# type 视角四选一（默认 busy）：
...
- **日程分享链接 vs 会议链接**：两者是不同事物，不可混用。
  - 日程分享链接：`https://<domain>/calendar/share?[REDACTED]`，指向日程本身，用于分享日程详情。**分享日程给某个人、某个群或粘贴到文档中，需要的都是这个日程分享链接（通过 `calendar events share_info` 获取），不是 applink**；禁止自己拼接 applink 或用 applink 代替。
...
# 返回形如 {{domain}}/calendar/share?[REDACTED] 的分享链接，不是 applink；直接把该链接发给对方（对方可凭链接中的 token 走 +join-event 加入）
