...
`AutomationUpdate` and `Widget.update` revalidate related Bindings. Inspect `revalidatedBindings` in the update response before running or enabling automatic work.
...
3. `AutomationControl` with `action: "readRun"` shows terminal `status: "succeeded"`.
