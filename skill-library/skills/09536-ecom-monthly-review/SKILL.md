---
name: ecom-monthly-review
trigger: ecom, monthly, review
description: 电商月度复盘分析工具，输入30天订单CSV，输出月度KPI报告、趋势分析、环比对比、问题诊断。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> ecom-monthly-review 是一个专业的 电商月度复盘分析工具，输入30天订单CSV，输出月度KPI报告、趋势分析、环比对比、问题诊断。，帮助用户快速完成相关任务并输出标准化交付物。
...
ecom-monthly-review 属于 **电商运营** 域（e-commerce operations for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 ecom-monthly-review 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(ecom-monthly-review)
    B --> C[分析结果]
    B --> D[解决方案]
```
