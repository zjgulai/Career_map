---
name: ab-testing
trigger: ab test, A/B测试, 对照实验, 分流测试, 假设检验
description: A/B测试设计与分析框架
category: dev
version: 2.1.0
author: hermes-ai
...
> ab-testing 是一个专业的 A/B测试设计与分析框架，帮助用户快速完成相关任务并输出标准化交付物。
...
ab-testing 属于 **开发运维** 域（development and DevOps for Momcozy），可与其他 skill 形成工作流链路：

- **ai-business-strategy**: 当本skill输出需要进一步处理时调用
...
```
使用 ab-testing 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(ab-testing)
    B --> C[ai-business-strategy]
    B --> D[hermes-ultraqa]
    B --> E[test-strategist]
```
