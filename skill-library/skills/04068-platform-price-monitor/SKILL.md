---
name: "platform-price-monitor"
title: "平台价格监控"
description: "当用户需要监控Amazon、eBay、AliExpress等电商平台竞品价格变化时使用。触发词：平台价格监控、价格监控、竞品价格追踪、Amazon价格预警、eBay比价、AliExpress监控、重定价策略、价格情报、动态定价。何时不用：非电商平台价格监控（如线下零售/B2B/股票/加密货币/汇率）、需要实时秒级更新、无API凭证且不接受Mock数据、仅需一次性价格查询、纯竞品分析/SEO/选品/广告/文案等非价格监控任务。缺不可推定的关键材料（账号/文件/数值）才追问；可依行业惯例或品牌既定风格推定的，标注假设后继续，绝不编造数据。"
user_summary: "自动盯住竞品价格：多平台监控，触发阈值就预警。不想每天手动比价时，找它。"
enabled: "true"
...
- 非电商platform-price-monitor（如线下零售、B2B平台）
...
| 维度 | ecommerce-price-monitor | amazon-competitor-monitor | 本 Skill（platform-price-monitor） |
|------|------------|-------------|----------------------|
...
- 需要 Amazon/eBay/AliExpress 三platform-price-monitor → 本 Skill
- 仅需跨平台价格对比单一功能 → ecommerce-price-monitor
- 需要 Amazon Listing 变更+促销监控 → amazon-competitor-monitor
- 需要多源competitor-profiling（定位/策略/融资）→ competitor-profiling
