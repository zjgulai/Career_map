...
trigger: ecom, daily, report
description: 电商日报/周报自动生成工具，输入1-7天订单CSV，输出日常经营报告。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> ecom-daily-report 是一个专业的 电商日报/周报自动生成工具，输入1-7天订单CSV，输出日常经营报告。，帮助用户快速完成相关任务并输出标准化交付物。
...
ecom-daily-report 属于 **电商运营** 域（e-commerce operations for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 ecom-daily-report 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(ecom-daily-report)
    B --> C[分析结果]
    B --> D[解决方案]
```
