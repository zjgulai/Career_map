---
name: SKILL
trigger: skill, strategy, insight/cross, domain,
...
> SKILL 是一个专业的 当 anomaly-detector 或其它 H 域分析产出异常事件、需要映射到产品/供应链/渠道/客服等具体 Skill id 并去重时调用；不自动执行下游，只输出路由建议与优先级。，帮助用户快速完成相关任务并输出标准化交付物。
...
```
skill, strategy, insight/cross, domain,
```
