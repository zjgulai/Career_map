...
title: "创建财务模型"
description: "- This skill provides an advanced financial modeling suite with DCF analysis, sensitivity testing, Monte Carlo simulations, and scenario planning for investment decisions 触发词：财务建模、DCF 估值、敏感性分析、蒙特卡洛模拟、情景规划、投资决策建模。"
disable-model-invocation: true
enabled: "true"
user-invocable: true
input_contract: 3-5年财报与关键假设（增长、利润率、折现率等）
output_contract: 完整财务模型：估值结论+敏感性表/龙卷风图+情景对比（可选蒙特卡洛分布）
example: 说「给这家公司建DCF估值模型」并给财报和假设 → 得到含企业价值、敏感性表的完整模型
...
## 触发词
- 创建财务模型、creating-financial-models、DCF、敏感性、蒙特卡洛等财务建模套件 等表述时使用。

## 何时使用
- DCF、敏感性、蒙特卡洛等财务建模套件。

## 何时不用
- DCF 估值专项走 dcf-valuation；电商预算预测走 ecommerce-budget-forecaster；利润率分析走 profit-margin-analyzer；市场 TAM 建模走 market-scenario-modeler
...
> 2026-09-07 SkillOpt b5 rollout：3 任务均分 94.7，轻量修复（元数据/何时不用/路由名/域名类）
