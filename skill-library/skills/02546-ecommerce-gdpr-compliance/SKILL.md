...
title: "电商 GDPR 合规管理"
description: "- Implement GDPR compliance for ecommerce including cookie consent, Subject Access Requests (SAR), right to erasure, and data processing agreements. 触发词：GDPR 合规、Cookie 同意、数据主体访问请求、被遗忘权、数据处理协议、电商隐私合规。"
disable-model-invocation: true
enabled: "true"
user-invocable: true
workflow: "建立处理活动记录（RoPA）；配置同意管理与 Cookie 横幅；响应主体访问请求（SAR、30 天窗口）；执行删除权（区分匿名化与删除）；签订数据处理协议（DPA）"
input_contract: 店铺平台与业务覆盖区域（可选：在用工具清单）
output_contract: 合规落地清单与分项设置指引（文档，实时）
example: 说「我的店开始服务欧盟客户」→ 得到 Cookie 同意、数据请求与删除权的落地清单和操作步骤。
...
## 触发词
- 电商 GDPR 合规管理、ecommerce-gdpr-compliance、Cookie 同意、SAR 请求与删除权等合规落地 等表述时使用。

## 何时不用
- 支付合规走 payment-fraud-detector；退货政策走 return-policy-designer；销售税合规走 sales-tax-vat-automator
