...
export const auth = {
  get[REDACTED] => SecureStore.getItemAsync(TOKEN_KEY),
  set[REDACTED] string) => SecureStore.setItemAsync(TOKEN_KEY, token),
  remove[REDACTED] => SecureStore.deleteItemAsync(TOKEN_KEY),
};

// Authenticated fetch wrapper
const authFetch = async (url: string, options: RequestInit = {}) => {
  const [REDACTED] auth.getToken();
...
const getValid[REDACTED] (): Promise<string> => {
  const [REDACTED] auth.getToken();
