...
trigger: file, history, manager
description: Claude Code 文件历史管理工具，支持查看、备份和恢复文件历史版本。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> File History Manager 是一个专业的 Claude Code 文件历史管理工具，支持查看、备份和恢复文件历史版本。，帮助用户快速完成相关任务并输出标准化交付物。
...
File History Manager 属于 **通用** 域（general-purpose for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 File History Manager 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(File History Manager)
    B --> C[分析结果]
    B --> D[解决方案]
```
