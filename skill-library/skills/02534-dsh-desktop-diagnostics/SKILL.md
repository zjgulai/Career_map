...
1. **查生命周期**：`~/Library/Application Support/DSH Desktop/lifecycle-events/startup.jsonl`，最后一条的 `finalStage`（`profile-composition` / `host-boot` / `health-commit`）与 `rendererStatus`。
2. **查日志**：`~/Library/Application Support/DSH Desktop/logs/dsh-<日期>.log`。
...
### 5. 应用品牌字符串替换（实测 LUTE Agentic System 重命名）

改 app 源码里的品牌字符串是**纯文本替换**（低风险），但有三条红线：

- ✅ 安全：原生 UI 页 JS（`lib/native-ui/assets/*.js` 的显示文案）、CLI 帮助文本、shell 显示名——纯显示字符串盲替换即可（备份先行）。
- ❌ **路径 join 字符串不可替换**：`bin.js` 里 `path.join(appData, "DSH Desktop")` / `Application Support", "DSH Desktop"` / `config, "DSH Desktop"` 是**数据目录路径**——改成新名字会让 CLI/诊断找不到数据目录。替换时用「只替换未加引号（显示文案）保留加引号（路径）」区分：`perl -pi -e 's/(^|[^"])DSH Desktop/$1新名字/g'`。
- ❌ **Info.plist 应用名不可改**：`CFBundleName` 改了会让 `app.getPath('userData')` 指向新目录 → 全部插件/会话/设置丢失（见工具箱 1 的 ❌ 清单）。
