...
description: |
  Create a stylized AI music video or emotional short with lyric typography from music, lyrics, references, characters, mood, and platform needs. Analyze beat and vocals, separate visual and text references, design beat-reactive type, break long works into connected shots, generate H3 clips by default, and assemble them against one master audio track. Deliver concepts, shot prompts, lyric plans, and a finished edit. Not for ordinary caption cleanup, licensed-IP copying, or fully manual editing.
trigger-words: [MV, music video, lyric typography, on-screen text, prompt audit, Trap MV, Gospel hip-hop, Dark-pop, Cyber-grunge]
---
...
- Keep implementation-specific calls and transport details outside the workflow.
- Write locked prompts and revisions to Design canvas text nodes.
- Produce character cards, scene cards, typography cards, video clips, BGM, and the final assembly as distinct stages with clear inputs and outputs.
- Do not hardcode output paths; always use Design-returned paths.
- If the user asks to skip confirmation or just proceed, apply it only to the current run unless the outer orchestrator has active session authorization.
...
- Use H3 as the recommended default for video generation. Switch only when the user explicitly requests another model, H3 is unavailable, or a hard capability requirement cannot be met.
...
4. **Generation and edit assembly**: generate each shot from the Shotlist, then align all clips to the global Master Audio beat grid, trimming, speed-ramping, and stitching for natural lip-sync, beat sync, and transitions.
...
After the complete MV prompt script is locked, write it to a dedicated canvas text node named `Complete MV Prompt` or `complete-mv-prompt`. The node must contain the full multi-shot prompt script and stitching notes. Later revisions update the same node.
...
2. **Parallel shot generation**: generate each 2–5s shot from the Shotlist. For continuation shots, extract the prior shot’s tail frame as the next I2V head frame.
3. **Multi-track editing and stitching**: import the global Master Audio, align all shot videos by timestamp, trim and speed-ramp on the beat grid, and check seams with same-direction motion or flash-cut masking.
