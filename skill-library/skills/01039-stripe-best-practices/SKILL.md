...
- *Always instantiate a `StripeClient` and call methods on that instance.* Do **not** use the deprecated global/module-level API key pattern (`stripe.[REDACTED]`, `Stripe.setApiKey`, `stripe.Key = …`, `StripeConfiguration.[REDACTED]`). The global pattern is deprecated in all current SDKs.
