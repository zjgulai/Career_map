---
name: cbec-product-attribute-analyzer
trigger: cbec, product, attribute, analyzer
description: 分析品类中哪些属性组合真正贡献销量，判断空白组合是伪机会还是真需求机会。当用户提及”属性分析”、”品类画像”、”属性组合”、”销量加权”、”市场结构”或需要产品结构机会分析时使用。 触发词：属性分析、
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> cbec-product-attribute-analyzer 是一个专业的 分析品类中哪些属性组合真正贡献销量，判断空白组合是伪机会还是真需求机会。当用户提及”属性分析”、”品类画像”、”属性组合”、”销量加权”、”市场结构”或需要产品结构机会分析时使用。 触发词：属性分析、，帮助用户快速完成相关任务并输出标准化交付物。
...
cbec-product-attribute-analyzer 属于 **跨境电商** 域（cross-border e-commerce for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 cbec-product-attribute-analyzer 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(cbec-product-attribute-analyzer)
    B --> C[分析结果]
    B --> D[解决方案]
```
