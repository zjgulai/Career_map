...
name: "chart-gen"
title: "图表图片生成"
user_summary: "把数据丢进来，得到一张能直接贴进文档的图表图片"
user_try: "把这 12 个月的销售额做成一张 PNG 折线图"
description: "用 Node 版 Vega-Lite 渲染、Sharp 输出 PNG 或 SVG 图表图片，含折线、柱状、K 线与热力图。触发词：生成图表、画个趋势图、做个柱状图、图表图片、导出 PNG、chart-gen。何时不用：要可交互的网页图表或整页报告用 lieflat-charts；文档内的结构关系图用 markdown-mermaid-writing。"
enabled: "true"
user-invocable: true
metadata:
  source: "third-party"
  ...
  sha256: "def86a5b0f7e931774e92b377b7c2b39938f5e620bea93b299156c4c1ff16ac5"
...
```bash
# 1. 生成图表
node chart.mjs --type line --data '...' --output /tmp/my-chart.png

# 2. 发送图片！使用 message 工具的 filePath 参数：
#    action=send, target=<channel_id>, filePath=/tmp/my-chart.png
```

**小贴士：**
- 保存到 `/tmp/`（持久化存储）而非 `/tmp/`（可能被清理）
