---
name: nextjs-ssr-hydration-fix
trigger: ssr fix, hydration, Next.js修复, 水合错误, 同构渲染
description: Next.js SSR修复
category: frontend
version: 2.1.0
author: hermes-ai
...
# nextjs-ssr-hydration-fix

> nextjs-ssr-hydration-fix 是一个专业的 Next.js SSR修复，帮助用户快速完成相关任务并输出标准化交付物。
...
```
ssr fix, hydration, Next.js修复, 水合错误, 同构渲染
```
...
nextjs-ssr-hydration-fix 属于 **开发运维** 域（development and DevOps for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 nextjs-ssr-hydration-fix 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(nextjs-ssr-hydration-fix)
    B --> C[分析结果]
    B --> D[解决方案]
```
