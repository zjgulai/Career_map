...
trigger: semantic, document, chunker
description: 智能文档分块器，保持语义边界，生成层次化的文本块结构。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> Semantic Document Chunker 是一个专业的 智能文档分块器，保持语义边界，生成层次化的文本块结构。，帮助用户快速完成相关任务并输出标准化交付物。
...
Semantic Document Chunker 属于 **通用** 域（general-purpose for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 Semantic Document Chunker 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(Semantic Document Chunker)
    B --> C[分析结果]
    B --> D[解决方案]
```
