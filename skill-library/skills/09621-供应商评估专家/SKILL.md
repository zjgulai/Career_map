---
name: SCM Supplier Evaluator - 供应商评估专家
trigger: scm, supplier, evaluator, , , 供应商评估专家
description: 1688背景调查、验厂评估、准入决策支持。提供供应商画像、风险评估与准入建议。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> SCM Supplier Evaluator - 供应商评估专家 是一个专业的 1688背景调查、验厂评估、准入决策支持。提供供应商画像、风险评估与准入建议。，帮助用户快速完成相关任务并输出标准化交付物。
...
SCM Supplier Evaluator - 供应商评估专家 属于 **供应链** 域（supply chain management for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 SCM Supplier Evaluator - 供应商评估专家 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(SCM Supplier Evaluator - 供应商评估专家)
    B --> C[分析结果]
    B --> D[解决方案]
```
