---
name: SCM Logistics Optimizer - 物流路线专家
trigger: scm, logistics, optimizer, , , 物流路线专家
description: 运输路径优化、成本时效平衡、承运商选择。提供最优物流方案与成本测算。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> SCM Logistics Optimizer - 物流路线专家 是一个专业的 运输路径优化、成本时效平衡、承运商选择。提供最优物流方案与成本测算。，帮助用户快速完成相关任务并输出标准化交付物。
...
SCM Logistics Optimizer - 物流路线专家 属于 **供应链** 域（supply chain management for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 SCM Logistics Optimizer - 物流路线专家 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(SCM Logistics Optimizer - 物流路线专家)
    B --> C[分析结果]
    B --> D[解决方案]
```
