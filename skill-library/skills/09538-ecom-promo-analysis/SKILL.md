---
name: ecom-promo-analysis
trigger: ecom, promo, analysis
description: 电商大促活动效果分析工具，输入活动期订单CSV，输出活动效果评估、ROI分析、流量转化分析、优化建议。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> ecom-promo-analysis 是一个专业的 电商大促活动效果分析工具，输入活动期订单CSV，输出活动效果评估、ROI分析、流量转化分析、优化建议。，帮助用户快速完成相关任务并输出标准化交付物。
...
ecom-promo-analysis 属于 **电商运营** 域（e-commerce operations for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 ecom-promo-analysis 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(ecom-promo-analysis)
    B --> C[分析结果]
    B --> D[解决方案]
```
