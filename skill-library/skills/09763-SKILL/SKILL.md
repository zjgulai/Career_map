---
name: SKILL
trigger: skill, channel, operate/product, label,
...
> SKILL 是一个专业的 当需要为包装与说明书生成各目标市场法规定位的标签文案草稿（符号、警示语、制造商信息、语言组合）且已具备认证缺口或技术文件要点时调用；印刷前须法务与公告机构确认。，帮助用户快速完成相关任务并输出标准化交付物。
...
```
skill, channel, operate/product, label,
```
