---
name: lute-skills-manage
trigger: lute, skills, manage
description: Claude Skills项目管理器。把不同项目的Claude Skills做成可切换、可按项目启停的统一管理器，解决100+Skills后的上下文污染和切换麻烦问题。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> lute-skills-manage 是一个专业的 Claude Skills项目管理器。把不同项目的Claude Skills做成可切换、可按项目启停的统一管理器，解决100+Skills后的上下文污染和切换麻烦问题。，帮助用户快速完成相关任务并输出标准化交付物。
...
lute-skills-manage 属于 **通用** 域（general-purpose for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 lute-skills-manage 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(lute-skills-manage)
    B --> C[分析结果]
    B --> D[解决方案]
```
