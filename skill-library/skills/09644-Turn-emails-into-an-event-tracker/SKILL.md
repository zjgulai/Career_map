...
trigger: turn, emails, into, an, event, tracker
description: Source: https://claude.com/resources/use-cases/turn-emails-into-an-event-tracker
...
> Turn emails into an event tracker 是一个专业的 Source: https://claude.com/resources/use-cases/turn-emails-into-an-event-tracker，帮助用户快速完成相关任务并输出标准化交付物。
...
Turn emails into an event tracker 属于 **通用** 域（general-purpose for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 Turn emails into an event tracker 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(Turn emails into an event tracker)
    B --> C[hermes-team]
    B --> D[hermes-ultraqa]
```
