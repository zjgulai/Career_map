...
description: >-
  Size a pre-launch Amazon ad test budget: keyword CPC, budget scenarios, expected vs breakeven vs target ACoS. Use when the user has a product, price and launch plan and asks about ad spend.
metadata:
  version: "1.0.0"
---
...
validation is `amazon-prelaunch-demand-check`'s job, and landed-cost / fee /
margin math is `amazon-prelaunch-margin-check`'s job. This skill consumes the outputs
