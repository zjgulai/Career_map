...
trigger: create, a, custom, webpage
description: Source: https://claude.com/resources/use-cases/create-a-custom-webpage
...
> Create a custom webpage 是一个专业的 Source: https://claude.com/resources/use-cases/create-a-custom-webpage，帮助用户快速完成相关任务并输出标准化交付物。
...
Create a custom webpage 属于 **通用** 域（general-purpose for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 Create a custom webpage 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(Create a custom webpage)
    B --> C[hermes-team]
    B --> D[hermes-ultraqa]
```
