...
description: 判断是否调用 `enterprise_agentic_search` 工具前，必须先完整读取 `doubao-enterprise-search` skill。适用场景包括：用户本轮输入包含 `[企业知识]` 或 `doubao-enterprise-search` 的 Skill 调用标记；问题明确或高度可能依赖企业内部资料、内部口径、内部系统、业务背景、历史讨论、协作记录、制度流程、员工权益、差旅报销，或内部产品、项目、客户、业务数据及非飞书通讯录 ID/UID/账号/标识；用户需要搜索、追溯、复盘或汇总会议、群聊、文档、日程、项目和人员相关信息，提炼结论、待办、风险、反馈或进展；用户需要结合公司、团队、项目、业务或协作背景完成办公写作、报告、方案、图表或其他内容创作；请求同时涉及企业内部信息和公开信息，或将会议、群聊、文档、日程、链接作为检索线索。
---
...
## 必读摘要（先读这里）
1. 显式入口下，信息获取请求必须调用；语义召回入口则结合信息来源和对象边界谨慎判断。详见第 1 章。
2. 同一检索目标默认只调用一次；围绕同一主题且可由同一批资料回答的子问题应合并，主题、对象或所需资料相互独立的问题应分别调用。query 必须是完整、可独立理解的自然语言。详见第 2 章。
3. 多轮对话可以继承上一轮的唯一主题和已确认约束，但不能继承显式入口的强制调用规则；来源切换后需要重新判断路由。详见第 3 章。
4. 无结果不能直接视为事实不存在；凡使用 `enterprise_agentic_search` 或 `lark-xxx` skill 获取的企业信息，即使后续通过 `doubao-visualization`、代码、文件工具或其他工具加工，仍必须遵守第 4.2 节的内容使用与输出格式要求和第 4.3 节的来源引用要求。输出前必须逐项检查，格式不符合要求或应引用而未引用时不得输出；`lark-xxx` 查询失败而问题本质仍是内部信息查询时，应使用 `enterprise_agentic_search` 兜底。详见第 4 章。
...
- 不是公开网页搜索工具；用户明确指定公开来源时，应走公开信息工具。
- 不替代 CLI、本地文件处理或直接回答。
...
- 显式入口（本轮输入包含 `[企业知识]` 或 `doubao-enterprise-search` 的 Skill 调用标记）→ 信息获取请求必须调用。
- 语义召回入口 → 结合信息来源和对象边界判断，边界不清时倾向不调用。

入口类型只对本轮生效。

**步骤 2：检查硬性排除（见 1.2）**

按 1.2 的具体规则检查硬性排除；命中后不调用 `enterprise_agentic_search`，而应导流到相应工具。
...
以上步骤仍无法确定 → 语义召回入口倾向不调用；显式入口的信息获取请求已在前述步骤确定为调用。

### 1.1 先判断入口类型
每次使用本 skill 时，先判断入口类型：

1. **显式入口**：用户本轮输入包含 `[企业知识]` 或 `doubao-enterprise-search` 的 Skill 调用标记，说明用户手动选择了企业搜索：只要未命中硬性排除，获取信息时都必须使用 `enterprise_agentic_search` 工具。
2. **语义召回入口**：本轮输入不包含上述 Skill 调用标记，但系统 / 模型因问题可能涉及企业内部资料而加载本 skill。说明当前只是“可能需要企业内部搜索”，本轮边界判断更谨慎：只有问题核心明确或高度可能依赖企业内部资料、内部口径、内部系统或企业协作记录时，才调用 `enterprise_agentic_search` 工具。

入口类型只对本轮生效；后续轮次不继承“显式入口”的强制调用规则，只能继承主题、来源方向和已确认约束。

### 1.2 硬性排除：命中即不调用 `enterprise_agentic_search` 工具
以下场景属于硬性排除，命中任一场景时不调用 `enterprise_agentic_search`，而应根据用户需求调用相应工具。对于显式入口，不得仅因所需信息可以从公开渠道获取，或问题属于医疗、金融、学术研究等专业垂直领域而排除；只有用户明确指定公开渠道或排除企业内部资料时，才按公开信息场景排除：
...
- 用户要求“问下某人 / 跟某人确认 / 找某人确认”等，且包含联系人和需要询问的问题时，属于消息发送操作，不调用本工具。
- 用户在本轮 query 中明确指定仅通过公开渠道获取信息，或明确要求不使用飞书、企业内部资料；不得根据问题能否从公开渠道获取而推断。
...
### 1.4 边界不清时的判断原则
本节只处理「未命中 1.2 硬性排除，也未命中 1.3 明确调用」的情况；适用于显式入口和语义召回入口，仍无法确定时再按 1.5 的默认倾向处理。
...
2. **再看信息来源**：
   ...
   - 用户在本轮 query 中明确指定公开来源或明确排除企业内部资料 → 公开搜索。
...
3. **再看是明确对象还是检索线索**：
   - URL/token/ID 或上下文中已唯一指向的飞书对象通常是明确对象；读取链接指向的对象本体这一步必须先走对应 `lark-xxx`。若用户请求中包含 `oc_xxxxxx` 格式的群 ID，但当前上下文中没有对应群名，必须先使用 `lark-im` 获取群的相关内容。该群 ID 既可能直接出现在用户输入中，也可能作为用户请求所指向的对象出现在 `<current-state></current-state>` 标签中。
   - 读取对象本体后按用户主目标分流：若目标是读取、总结、提取对象本体中的字段、名单、人员、机构、金额、状态、结论或正文内容，且对象内容足以回答，继续走 `lark-xxx`，不调用 `enterprise_agentic_search`；若对象内容不足，或目标是查对象相关主题的背景、后续讨论、风险、决策依据、落地情况、跨来源反馈或内部补充资料，则 URL/token/ID 才作为检索线索，调用 `enterprise_agentic_search` 工具。
...
### 1.5 边界不清时的默认倾向
两类入口的默认倾向不同：
- **显式入口**：未命中第 1.2 节硬性排除，且经第 1.4 节判断后仍无法确定时，只要用户的目标是获取信息，就必须调用 `enterprise_agentic_search`。即使问题属于医疗、金融、学术研究等专业垂直领域，或存在更匹配该领域的专业搜索工具，也必须先调用 `enterprise_agentic_search`。但显式入口不能覆盖明确对象规则：用户提供飞书 URL 并围绕该对象本体提问时，必须先使用对应的 `lark-xxx` 读取对象，不得直接将该 URL 作为 `enterprise_agentic_search` 的检索线索。群 ID 的信息补全同样需要使用 `lark-im` 补全具体信息。
- **语义召回入口**：本轮输入不包含 Skill 调用标记，只是系统 / 模型判断可能需要企业内部搜索。边界不清默认更谨慎；只有问题核心明确或高度可能依赖企业内部资料、内部口径、内部系统或企业协作记录时，才调用 `enterprise_agentic_search` 工具。

以下情况同样属于信息获取，在**显式入口**下必须调用 `enterprise_agentic_search` 工具：
...
**调用方式（强制遵循）**
1. 每次调用必须单独占用一轮：同一轮只能调用一次 `enterprise_agentic_search`，不得同时调用其他工具，也不得并行发起多个 `enterprise_agentic_search` 请求。该工具返回内容通常较长，必须等待并完整保留本次结果后，再发起下一次调用或执行其他工具。
2. 先按检索目标确定调用次数：围绕同一主题、依赖相同上下文且可由同一批资料回答的多个内部子问题，应合并为一次调用；主题、对象或所需资料相互独立的问题，应分别调用。同一检索目标默认只调用一次，仅在以下情况可以或必须补查：
   - 后续查询明确依赖首次结果中的新实体、新线索或新材料；
   - 用户对该检索目标新增或澄清了查询要求；
   - 对于有限候选集，首次结果未覆盖全部必要候选对象，此时必须针对未覆盖对象补查，不得用“信息不足”或免责声明代替。
3. 补查只能包含缺失对象、新增要求或新线索，不得重复已覆盖范围；补查后仍无信息时，才能将对应对象标记为“未确认”。不得仅因结果泛泛不足、无结果、想查全一点、变换关键词或补充公开信息后再确认而重复调用。
...
### 2.3 调用标记不传入 query
如果用户本轮输入包含 `[企业知识]` 或 `doubao-enterprise-search` 的 Skill 调用标记，传入 `enterprise_agentic_search` 工具时，应剥离该调用标记及其关联路径，只保留用户的实际 query。
...
但入口类型不继承：显式入口的强制调用规则只在当轮生效。后续轮次如果用户输入不再包含 `[企业知识]` 或 `doubao-enterprise-search` 的 Skill 调用标记，则按语义召回边界判断；但可以继承上一轮已经确认的主题、来源方向和约束条件。
...
`enterprise_agentic_search` 或 `lark-xxx` skill 返回的是企业内部资料的参考内容，可能是已整理信息，也可能是相关片段和来源信息。后续调用 `doubao-visualization` 等 skill 回答、加工或展示这些企业知识时，也必须遵守本节要求。使用结果时遵循：
- 结论前置，优先回答用户最关心的问题，再补充依据、来源和必要背景。
- 最终回答应使用与用户相同的语言，除非用户明确要求使用其他语言。
- 优先保留工具返回的原文内容、表述顺序和上下文，尤其保留关键表述和发言主体；仅为直接回答用户问题进行必要的筛选、压缩、合并或重排，避免大范围改写。不得改变原意、扩大范围，或补充返回片段未明确支持的事实、因果关系、判断和假设。
...
- `enterprise_agentic_search` 返回图片时，应结合图片的 caption 判断其与用户问题的相关性及理解价值，主动选择并展示能直接回答用户问题、承载关键信息或显著帮助理解的相关图片，不要无故省略或只输出文字摘要、图片标题；展示时必须直接使用 Markdown 图片语法 `![{图注}]({图片URL})`，不得调用 `present_files` 交付，也不得使用普通 Markdown 链接或裸 URL 代替；每张图片只输出一次。

### 4.3 企业问答引用与材料链接要求（重要信息，强制遵循）
**输出前强制检查**
1. 逐项检查正文、列表、表格、图片说明和交付物说明中是否使用了 `enterprise_agentic_search` 或 `lark-xxx` skill 返回的内容。
2. 每项被使用的内容都必须按照本节规定的位置添加对应的 `<RichMediaReference>`；Markdown 材料链接、裸 URL 或集中来源列表不能代替事实引用。
3. 只要存在应引用但未引用的内容，就必须先补齐引用，不得输出。
...
- **来源关系必须贯穿后续产出**：凡基于 `enterprise_agentic_search` 返回内容生成、加工或计算的回答或其他交付物，即使后续调用 `doubao-visualization` 等 skill，最终回复都必须保留其所依据内容的来源引用；对于涉及企业知识的交付物，应在最终回复对产物内容的说明中添加对应引用。
...
- **表格引用**：输出表格前，必须根据各项事实的来源判断应采用以下哪种引用模式，并检查表格是否符合对应格式；如不符合，必须修正后再输出。不得在表格后另起一行放置用于支撑该表的引用。
  1. **表格涉及多个来源**：禁止统一引用；每项事实的引用必须紧跟在对应单元格内容后。一个单元格由多个来源共同支撑时，将全部来源合并到该单元格的同一个 `<RichMediaReference>` 中。
  2. **整表仅涉及一个来源**：在表格描述后、表格之前统一引用，无需在单元格内重复引用。
- **生成图片与图表**：通过 `doubao-visualization` 或其他工具生成图片、图表时，生成结果本身不能代替来源引用。最终回复必须先说明图片或图表表达的主要企业信息，并在对应说明后就地添加 `<RichMediaReference>`，再展示或交付生成结果；不得只输出图片、文件链接或“已生成”等无引用说明。使用多个企业信息来源时，说明必须覆盖并引用对应来源。
- **图片与图注引用**：引用必须紧跟在图注后、Markdown 图片前；不要把引用放在图片后面，也不要在图片下方重复输出图注。格式为：`1. {图注}<RichMediaReference>["{来源URL}"]</RichMediaReference>`，换行后输出 `   ![{图注}]({图片URL})`。`{来源URL}` 使用图片所属或相邻 `<document>` 的 `<url>`。

#### 输出格式示例
**1. 正文引用与材料入口**

正确：引用紧跟事实，材料入口单独保留且不重复引用。
```markdown
事实内容<RichMediaReference>["URL1","URL2"]</RichMediaReference>
材料：[材料标题](URL)
```
错误（以下三组为相互独立的错误示例，依次为引用另起一行、Markdown 链接后重复引用、普通 URL 后重复引用）：
```markdown
事实内容
...
工具返回的 URL 可能包含查询参数和文档锚点，必须逐字符完整迁移：
```text
...
## 5. 易混边界速查
- **文档**：给出明确文档 / 文件 / Wiki URL 时，读取链接指向的对象本体这一步必须先走 `lark-xxx`；打开、读取、总结、提取字段/名单/正文内容、编辑、导出明确文档本体 → `lark-xxx`。若读取后对象内容不足以回答，或主目标是围绕主题、项目或文档名查背景、讨论、反馈、风险、结论、决策依据、后续进展，才调用 `enterprise_agentic_search` 工具。
- **群聊 / 消息**：发送、回复、点赞、批量发送、@我、私聊、未回复、未读、查看群成员 / 群人数 / 群成员列表、基于消息 ID / 链接 / 时间点定位某条明确消息，或明确要求问下某人、跟某人确认、找某人确认 → `lark-im`；按主题、关键词、事项、结论或来源追溯谁说过、谁提过、谁反馈过，或汇总已有发言、讨论、观点、结论 → 调用 `enterprise_agentic_search` 工具。
