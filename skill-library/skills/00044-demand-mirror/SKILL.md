...
name: demand-mirror
description: >-
  Mirror any product onto Amazon demand signals: category size, trend, seasonality, and variant demand signals (never per-variant unit sales). 用于评估某商品在 Amazon 的需求多大、何时是高峰、备哪些颜色尺码（如独立站/其他渠道选品）。
---
...
chart titles/axes/legends, confidence labels and variant explanations. English
examples are not fixed output text: in Chinese, `Monthly trend` becomes “月度趋势”
and `Medium` becomes “中”. Preserve required JSON/CSV contract fields and enums,
source/brand names, actual search keywords and standard abbreviations. Localize
display labels without changing the underlying market, units, calculations or
the distinction between variant demand signals and sales.
