---
name: SKILL
trigger: skill, strategy, insight/anomaly, detec
...
> SKILL 是一个专业的 当需要基于销量、ACOS、库存、退货率等阈值发现异常，并建议触发产品/供应链/渠道域的哪个 Skill 时调用；不替代人工确认是否执行触发。，帮助用户快速完成相关任务并输出标准化交付物。
...
```
skill, strategy, insight/anomaly, detec
```
