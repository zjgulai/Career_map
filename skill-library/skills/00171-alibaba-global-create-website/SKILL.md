---
name: alibaba-global-create-website
version: "2.0.0"
description: |
  Canvas-based visual website builder that generates, previews, and validates React/Vite sites via the `workctl` CLI (accio-cli site builder commands).
  Covers five sub-capabilities: merchant info collection, Canvas design planning, code generation, local runtime, and safety gates. Supports creating new sites, redesign, bug fixes, and publishing.
enabled: true
...
- `references/verification.md` governs rendered preview verification — ask the user whether to auto-verify via browser, or let them check via canvas and report issues. Runs after `site-preview-start` and before the Delivery Gate.
...
- **Site Publish** (this document, § Publish Flow below) governs publishing the built site to the Alibaba online platform. Publishing calls the `ai_minisite_acciowork_publish_page` MCP tool **directly** (NOT `workctl icbu storefront site-publish`, which intercepts/blocks GGS merchants); version listing still uses `workctl icbu storefront site-page-version-list`.
...
1. Classify the request: static marketing site, landing page, redesign, bug fix, or copy edit.
...
11. Follow the `verification` subskill (`references/verification.md`): ask the user whether they want automatic browser verification. If yes → navigate to the preview URL via browser tools, check for runtime errors, verify rendered content and styling, auto-fix if issues found. If no → tell the user they can check via the canvas panel and report issues in chat for auto-fix. Do NOT skip this step silently.
...
| "Publish / deploy / go live / 发布 / 上线" | **Publish Flow** (see § below) | Ask new vs existing version, then build and call the `ai_minisite_acciowork_publish_page` MCP tool directly (bypasses the GGS-blocking `site-publish`). |
...
Task 8: Install, Build & Preview
    ↓ (blocks)
Task 8.5: Rendered Preview Verification (ask user: auto-verify or canvas check) + ES Module Import Compliance Check
    ↓ (blocks)
...
task_create({
  ...
  description: "Ask user verification preference, then verify page renders correctly via browser or skip for manual canvas check. MANDATORY: run code-level ES Module import compliance scan on all .ts/.tsx files",
...
// Classify by feature set: static site, landing page, redesign, bug fix.
...
//      - If errors found → fix source file → re-verify
//   3. If user chose manual: tell them to check canvas, report issues in chat
//   4. MANDATORY: Run code-level ES Module import compliance check
//      - grep all .ts/.tsx for hardcoded local image paths
//      - Fix violations: convert string paths to import statements
//      - Re-build if any violations were fixed
//   5. Do NOT proceed until verification complete or user-chosen skip
...
Then publish a **new version** (no page id) by following **Step P2c: Publish Execution** below with `pageId` omitted.

### Step P2b: If User Chooses "Existing Version"

First, fetch all existing page versions (this listing command is read-only and is NOT intercepted, so keep using `workctl`):
...
**Important**: Each option label MUST show both `pageName` and `pageId` so the user can clearly identify the target version.

Then ask for publish type (same as Step P2a), and publish to the selected `pageId` by following **Step P2c: Publish Execution** below.

### Step P2c: Publish Execution (Direct MCP Call — GGS-safe)

> **Why not `site-publish`?** `workctl icbu storefront site-publish` intercepts and blocks GGS merchants. To support GGS, call the underlying `ai_minisite_acciowork_publish_page` MCP tool **directly** — it performs the same publish without the GGS gate. Everything before this point (design, code generation, install, build, preview, verification) is unchanged; ONLY this final publish call changes. Do **NOT** run `workctl icbu storefront site-publish`.

Both Step P2a (new version) and Step P2b (existing version) converge here:

1. Ensure a fresh build (this replaces the build that `site-publish` used to run internally; `site-build` is not intercepted):

   ```bash
   workctl icbu storefront site-build --cwd <project-path>
   ```

2. Read the compiled HTML from `<project-path>/dist/index.html`. This MCP tool accepts the HTML content directly — do **NOT** pre-upload to OSS.

3. Call the `ai_minisite_acciowork_publish_page` MCP tool **directly** with these inputs:
   - **HTML content** — the full contents of `dist/index.html` from step 2.
   - **Publish type** — the target 端 (terminal) chosen by the user: `desktop` (PC) / `wireless` (无线) / `all` (双端).
   - **Page id** — the selected `pageId` **only for the "existing version" path (P2b)**. For a **new version (P2a), omit the page id** so the platform creates a new page.

   Inspect the `ai_minisite_acciowork_publish_page` tool's own input schema first to get the exact parameter names, then map: HTML content → its html/content parameter, publish type → its publish-type / terminal parameter, page id → its page-id parameter. Provide only the parameters the schema defines.

### Step P3: Report Result

After the `ai_minisite_acciowork_publish_page` MCP call returns, report the result to the user:
- On success → show the publish details from the MCP response (e.g., page id, publish result, and any returned page/preview URL).
- On failure → show the error message and suggest fixes (build error → fix code and re-run `site-build`; MCP / connectivity error → retry or check the MCP connection).
...
**IMPORTANT**: Each option label MUST show both `pageName` AND `pageId` together. This serves two purposes:
1. Helps the user identify the correct page (pageName is human-readable)
2. Ensures the agent knows the exact pageId for the selected version
...
- **Do not skip rendered preview verification silently** — always ask the user first (auto-verify or canvas check). If the user declines auto-verification, record the skip and tell them to check via canvas and report issues.
- **Do not claim a site is "complete" based on build success alone** — build success does not prove the page renders correctly. Run the verification gate or record the user's skip decision.
