...
title: "B2B付款账期优化"
description: "- Establish and manage B2B trade credit, net-terms, and automated invoicing to accelerate sales cycles while mitigating credit risk 触发词：B2B付款账期、贸易信用、Net30账期、信用额度、应收账款自动化、早付折扣。"
...
enabled: "true"
user-invocable: true
workflow: "配置平台原生 B2B 功能（信用额度与付款条款）；实施技术信用检查（Stripe 账单与额度校验）"
input_contract: 客户层级与信用情况（可选：所在平台）
output_contract: 按客户分层给账期/额度方案+风险预警标准与催收节奏，对话内表格+步骤，即时
example: 说「给老客户设 Net 30、新客户怎么定」→ 得到分层账期、额度与预警规则
...
## 触发词
- B2B付款账期优化、b2b-payment-terms-optimizer、净账期、信用额度与自动开票 等表述时使用。

## 何时不用
- 谈判策略走 sales-negotiator；发票生成走 invoice-generator；财务模型走 creating-financial-models
...
> 2026-09-07 SkillOpt b5 rollout：3 任务均分 89.0，轻量修复
