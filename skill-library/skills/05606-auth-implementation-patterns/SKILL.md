...
// Verify JWT
function verifyToken([REDACTED] JWTPayload {
    try {
...
class RefreshTokenService {
    ...
    async storeRefreshToken(userId: string, [REDACTED] {
        const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
        await db.refreshTokens.create({
            [REDACTED] hash(refreshToken),  // Hash before storing
            userId,
            expiresAt,
        });
    }
    ...
    async refreshAccessToken([REDACTED] {
        ...
        const stored[REDACTED] db.refreshTokens.findOne({
            where: {
                [REDACTED] hash(refreshToken),
    ...
    async revokeRefreshToken([REDACTED] {
...
app.get(
    ...
    (req, res) => {
        ...
        res.redirect(`${process.env.FRONTEND_URL}/auth/callback?[REDACTED]`);
    }
);
```
...
// Hash password
async function hashPassword([REDACTED] Promise<string> {
