...
trigger: prepare, for, sales, deals
description: Source: https://claude.com/resources/use-cases/prepare-for-sales-deals
...
> Prepare for sales deals 是一个专业的 Source: https://claude.com/resources/use-cases/prepare-for-sales-deals，帮助用户快速完成相关任务并输出标准化交付物。
...
Prepare for sales deals 属于 **通用** 域（general-purpose for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 Prepare for sales deals 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(Prepare for sales deals)
    B --> C[hermes-team]
    B --> D[hermes-ultraqa]
```
