...
title: "评论分析助手"
description: "- Use this skill to analyze product reviews, find common issues, and prioritize improvements. Triggers: \"analyze reviews\", \"review analysis\", \"customer feedback\", \"what are people saying\", \"product reviews\", \"review sentiment\", \"find complaints\", \"customer complaints\", \"improvement recommendations\", \"voice of customer\", \"VOC analysis\", \"feedback analysis\" Outputs: Prioritized issues, sentiment analysis, improvement recommendations."
disable-model-invocation: true
enabled: "true"
user-invocable: true
workflow: "逐问确认产品与评论来源；抓取评论数据；四个专业代理多角度分析；合成优先级矩阵与行动计划"
input_contract: 产品名/链接+评论来源（可选：已知问题），逐问确认
output_contract: 评论分析报告：情感、投诉排行、好评、优先级矩阵+行动计划（耗时）
example: 说「分析我们产品的Amazon评论」→ 得到投诉优先级+改进行动计划
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/正文内置报告生成流程（PDF 导出标注为可选依赖） \
...
## 触发词
- 评论分析助手、review-analyst-agent、大规模分析产品评论，找出共性问题与改进优先级 等表述时使用。

## 何时使用
- 大规模分析产品评论，找出共性问题与改进优先级。

## 何时不用
- 客户声音六维映射走 customer-voice-analyzer；情感分析走 voc-sentiment-analyzer；多平台评论采集走 product-review-intelligence-collector
