...
title: "客户生命周期价值计算器"
description: "- Trigger: Calculate customer lifetime value (CLV) and use it to drive retention strategies, VIP programs, and win-back flows. 边界：LTV 计算与客户分层依据；具体活动设计走 retention/winback"
...
whenToUse: "LTV 计算与客户分层依据；具体活动设计走 retention/winback"
enabled: "true"
user-invocable: true
workflow: "区分历史LTV与预测LTV；从平台导出客户数据；按LTV分层设计自动化流程；按决策标准深化应用"
input_contract: 客单价+年购买次数+客户留存年限+毛利率（可选：订单明细数据）
output_contract: 历史/预测 LTV 数值+可承受获客成本+回本周期+客户分层触发建议，即时
example: 说「客单价 100、一年买 4 次、毛利 50%」→ 得到 LTV 估算与获客成本上限
...
## 触发词
- 客户生命周期价值计算器、customer-ltv-calculator、计算历史与预测 LTV，驱动留存与召回 等表述时使用。

## 何时不用
- RFM 分群走 customer-rfm-analyzer；留存自动化走 customer-retention-automator；忠诚度设计走 loyalty-points-designer
