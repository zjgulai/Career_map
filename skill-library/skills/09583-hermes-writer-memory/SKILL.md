---
name: hermes-writer-memory
trigger: writer memory, 写作状态, 角色设定, 章节进度, 长期写作
description: 长期写作项目管理
category: creative
version: 2.1.0
author: hermes-ai
...
# hermes-writer-memory

> hermes-writer-memory 是一个专业的 长期写作项目管理，帮助用户快速完成相关任务并输出标准化交付物。
...
```
writer memory, 写作状态, 角色设定, 章节进度, 长期写作
```
...
hermes-writer-memory 属于 **开发运维** 域（development and DevOps for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 hermes-writer-memory 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(hermes-writer-memory)
    B --> C[分析结果]
    B --> D[解决方案]
```
