---
name: SEO Technical Audit - 技术SEO审计工具
trigger: seo, technical, audit, , , 技术seo审计工具
description: 支持多种审计引擎：Lighthouse、PageSpeed Insights API、自定义爬虫。提供网站结构分析、页面速度检测、移动端适配检查。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> SEO Technical Audit - 技术SEO审计工具 是一个专业的 支持多种审计引擎：Lighthouse、PageSpeed Insights API、自定义爬虫。提供网站结构分析、页面速度检测、移动端适配检查。，帮助用户快速完成相关任务并输出标准化交付物。
...
SEO Technical Audit - 技术SEO审计工具 属于 **搜索优化** 域（SEO/GEO optimization for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 SEO Technical Audit - 技术SEO审计工具 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(SEO Technical Audit - 技术SEO审计工具)
    B --> C[分析结果]
    B --> D[解决方案]
```
