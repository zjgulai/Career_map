---
name: hermes-team
trigger: team, 团队编排, 并行执行, 多代理, N个一起做, 协作, 编排
description: 多代理并行执行
category: ai
version: 2.1.0
author: hermes-ai
...
> hermes-team 是一个专业的 多代理并行执行，帮助用户快速完成相关任务并输出标准化交付物。
...
```
team, 团队编排, 并行执行, 多代理, N个一起做, 协作, 编排
```
...
hermes-team 属于 **开发运维** 域（development and DevOps for Momcozy），可与其他 skill 形成工作流链路：

- **hermes-ultrawork**: 当本skill输出需要进一步处理时调用
- **cbec-trend-timing-analyzer**: 当本skill输出需要进一步处理时调用
- **hermes-release**: 当本skill输出需要进一步处理时调用

## 一句话调用

```
使用 hermes-team 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(hermes-team)
    B --> C[hermes-ultrawork]
    B --> D[cbec-trend-timing-analyzer]
    B --> E[hermes-release]
```
