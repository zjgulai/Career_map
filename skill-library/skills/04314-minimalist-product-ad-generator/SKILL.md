...
description: |
  Turn product images into a minimalist ad with preserved identity, concise English copy, independent anchors, a beat storyboard, and music replacement. Not for talking-head ads or screen demos.
trigger-words: [minimalist product ad, premium product ad, minimalist product film, Apple-style product ad, ecommerce product film, product launch ad]
---

# Minimalist Product Ad Generator

Use this Skill to guide non-professional users through a flow-style workflow that creates an Apple-inspired product advertising video. The target users are e-commerce sellers, small brand owners, indie creators, and individual sellers; the user should provide at least one product image or related asset.
...
1. **Product images and related materials**
   ...
   - If the user has already provided materials, state that they have been detected. Do not ask the user to upload again; only confirm whether to use them.
...
2. **Product variant status**
   ...
   - If the user does not specify, recommend one based on the product image, but the later storyboard must still write the concrete color or variant name.
...
6. **In-frame copy**
   - User-provided copy, or workflow-generated Apple-style English copy.
...
7. **Video model strategy**
   ...
   - If the user-selected model fails, allow at most one targeted retry. Then explain the capability difference and let the user choose MiniMax H3 or another equivalent instead of switching silently.

Start gate is mandatory. If the user has already uploaded product material, only skip the upload request; do **not** skip the rest of the start gate. Still confirm product variant status, target duration, aspect ratio, Apple-style template, and in-frame copy mode in one pass before analysis, anchor generation, storyboard, or video generation. The video model is not a start-gate question: prefer MiniMax H3 and display it in the completed result. If the user says "you decide / just do it," use recommended defaults for the remaining fields, but still display the adopted choices before continuing.
...
- In-frame copy mode: user-provided copy, or workflow-generated Apple-style English copy.
...
- If the user does not specify a main variant, choose the most visually stable one and use the others for rhythm and layering.
...
- Copy splitting may only be described in plain language as first half / second half. Do not write arrow symbols, slashes, plus signs, or separators, because the model may render them as in-frame copy.
...
- State whether native audio is desired and include the confirmed music direction; use native audio by default unless the user asks for silence or another model.
- Do not pass the original product image.

## STEP 9: Standalone Tech BGM

Prefer MiniMax H3 native audio first because it can align product motion and sound in one pass. If the music is unpleasant, too loud, too weak, out of sync, or the user asks for new music, create a longer standalone instrumental with an available music model and replace the video audio track. Confirm any material cost or output tradeoff before choosing a paid or tightly duration-controlled option.
...
- Final outputs are organized, and multiple assets are clearly grouped.

Delivery includes:

- Final video or its accessible output location.
...
- Music quality is poor: create a longer instrumental alternative, then cut its best continuous segment instead of forcing an exact duration.
...
- "Turn this product image into a minimalist launch ad"
- "Make a premium ecommerce product video"
- "Create a clean Apple-inspired product short"
