...
with Agent.resume(
    previous_agent_id,
    AgentOptions([REDACTED]"CURSOR_API_KEY"]),
) as agent:
...
```bash
export CURSOR_[REDACTED]"  # user API key or team service-account key
```
...
const models = await Cursor.models.list({ [REDACTED] });
```
