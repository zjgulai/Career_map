---
name: hermes-omc-doctor
trigger: omc doctor, 诊断, 安装诊断, 环境检查, 故障排查, 健康检查
description: OMC/Hermes安装诊断
category: devops
version: 2.1.0
author: hermes-ai
...
> hermes-omc-doctor 是一个专业的 OMC/Hermes安装诊断，帮助用户快速完成相关任务并输出标准化交付物。
...
```
omc doctor, 诊断, 安装诊断, 环境检查, 故障排查, 健康检查
```
...
hermes-omc-doctor 属于 **开发运维** 域（development and DevOps for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 hermes-omc-doctor 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(hermes-omc-doctor)
    B --> C[分析结果]
    B --> D[解决方案]
```
