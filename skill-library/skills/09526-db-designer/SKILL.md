---
name: db-designer
trigger: db design, 数据库设计, schema设计, 表结构, 索引优化, 规范化
...
> db-designer 是一个专业的 数据库schema设计，帮助用户快速完成相关任务并输出标准化交付物。
...
```
db design, 数据库设计, schema设计, 表结构, 索引优化, 规范化
```
...
db-designer 属于 **开发运维** 域（development and DevOps for Momcozy），可与其他 skill 形成工作流链路：

- **perf-optimizer**: 当本skill输出需要进一步处理时调用
...
```
使用 db-designer 帮我下一步行动或相关skill推荐...
```
