...
metadata:
  ...
  requires:
    ...
    env:
      - "GUAIKEI_API_TOKEN"
  env_desc:
    GUAIKEI_API_[REDACTED] API 访问令牌。未配置时无法调用接口；可通过 www.guaikei.com 在线自助开通。"
  category:
    - "Data&APIs"
    - "内容创作"
    - "数据分析"
...
抖音公开数据只读获取工具。支持**关键词搜索排序**、**博主作品抓取**、**视频/图文评论抓取**、**实时热榜获取**，输出结构化 JSON，适用于短视频选题、竞品监控、舆情分析、热点追踪。
