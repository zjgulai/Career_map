...
Launch a single reviewer subagent using the `task` tool with `fork_context: true` so it inherits the full conversation context.
...
- **fork_context=true on the reviewer launch** -- the reviewer needs the conversation context (the user's framing, any constraints, etc.). Trade-off: a long parent session inflates the reviewer's input tokens, and PR mode adds the diff on top of that, so users running `/review --pr` from a heavily-used session may want to `/clear` first to keep latency and cost down.
