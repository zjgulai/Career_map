...
name: blog-post
description: |
  A long-form article / blog post — masthead, hero image, article body
  with figures and pull quotes, author byline, related posts.
...
3. **Sections**, in order:
   ...
   - **Hero image** — a 16:9 **real image** (never an SVG/CSS placeholder or
     gradient block): default to a model-generated image matching the
     article topic; when the article is about a specific real subject
     (named brand / public figure / place / product), use a real image the
     model saw in pre-training; or the user's own uploaded asset. Add a
     1-line caption underneath.
   - **Body** — alternating prose paragraphs with at least:
     ...
     - 1 figure (a real image — model-generated or a real subject per the rule above — plus caption).
     - 1 list (numbered or bulleted).
     - 1 inline blockquote.
   - **Author footer** — author avatar (initials in a circle), bio paragraph.
   - **Related** — 3 cards linking to other posts. Each card: tiny real
     thumbnail image (model-generated or a real subject per the hero rule —
     no gradient/SVG placeholder), title, 1-line excerpt, date.
