...
trigger: mece, knowledge, extractor
description: MECE 原则知识提取器，从文本中提取结构化知识单元（SKU）。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> MECE Knowledge Extractor 是一个专业的 MECE 原则知识提取器，从文本中提取结构化知识单元（SKU）。，帮助用户快速完成相关任务并输出标准化交付物。
...
MECE Knowledge Extractor 属于 **通用** 域（general-purpose for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 MECE Knowledge Extractor 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(MECE Knowledge Extractor)
    B --> C[分析结果]
    B --> D[解决方案]
```
