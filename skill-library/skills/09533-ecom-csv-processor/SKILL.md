---
name: ecom-csv-processor
trigger: ecom, csv, processor
description: 电商订单CSV数据预处理工具，支持字段映射、数据清洗、格式标准化。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> ecom-csv-processor 是一个专业的 电商订单CSV数据预处理工具，支持字段映射、数据清洗、格式标准化。，帮助用户快速完成相关任务并输出标准化交付物。
...
ecom-csv-processor 属于 **电商运营** 域（e-commerce operations for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 ecom-csv-processor 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(ecom-csv-processor)
    B --> C[分析结果]
    B --> D[解决方案]
```
