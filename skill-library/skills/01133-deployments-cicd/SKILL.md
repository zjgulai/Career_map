...
```bash
VERCEL_[REDACTED]        # Personal or team token
  ...
  deploy:
    ...
    steps:
      ...
      - name: Pull Vercel Environment
        run: vercel pull --yes --environment=production --[REDACTED] secrets.VERCEL_TOKEN }}

      - name: Build
        run: vercel build --prod --[REDACTED] secrets.VERCEL_TOKEN }}

      - name: Deploy
        run: vercel deploy --prebuilt --prod --[REDACTED] secrets.VERCEL_TOKEN }}
```
...
**What OIDC does not do:** Authenticate the Vercel CLI in CI pipelines. All `vercel pull`, `vercel build`, and `vercel deploy` commands still require `--[REDACTED] secrets.VERCEL_TOKEN }}`.
...
deploy:
  ...
  script:
    ...
    - vercel pull --yes --environment=production --[REDACTED]
    - vercel build --prod --[REDACTED]
    - vercel deploy --prebuilt --prod --[REDACTED]
  only:
    - main
```
...
jobs:
  preview:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: npm install -g vercel
      - run: vercel pull --yes --environment=preview --[REDACTED] secrets.VERCEL_TOKEN }}
      - run: vercel build --[REDACTED] secrets.VERCEL_TOKEN }}
      - id: deploy
        run: echo "url=$(vercel deploy --prebuilt --[REDACTED] secrets.VERCEL_TOKEN }})" >> $GITHUB_OUTPUT
      - name: Comment PR
...
jobs:
  ...
  promote:
    ...
    steps:
      - run: npm install -g vercel
      - run: vercel promote ${{ needs.deploy-preview.outputs.url }} --[REDACTED] secrets.VERCEL_TOKEN }}
```
