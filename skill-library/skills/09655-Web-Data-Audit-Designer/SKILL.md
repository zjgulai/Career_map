---
name: Web Data Audit Designer
trigger: web, data, audit, designer
description: 基于真实网页样本审计，设计低维护成本、可支撑业务分析的数据采集方案。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> Web Data Audit Designer 是一个专业的 基于真实网页样本审计，设计低维护成本、可支撑业务分析的数据采集方案。，帮助用户快速完成相关任务并输出标准化交付物。
...
Web Data Audit Designer 属于 **通用** 域（general-purpose for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 Web Data Audit Designer 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(Web Data Audit Designer)
    B --> C[分析结果]
    B --> D[解决方案]
```
