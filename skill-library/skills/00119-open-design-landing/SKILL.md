...
inputs:
  ...
  - id: imagery
    label: Image strategy (generate [default] / bring-your-own / placeholder [deprecated])
...
parameters:
  ...
  image_strategy:
    type: enum
    values: [generate, bring-your-own, placeholder]
    default: generate
    description: >
      `generate` (default) calls gpt-image-2 (fal.ai or Azure) for all 16
      slots — real, original images. `bring-your-own` assumes the user has
      dropped 16 real images at `imagery.assets_path` (use this when the
      user wants a specific real subject — named brand / public figure /
      landmark). `placeholder` is DEPRECATED: it writes SVG placeholder
      frames, which are banned by `craft/anti-ai-slop.md`; only use it as a
      throwaway local wireframe, never for anything the user will see.
  image_provider:
    type: enum
    values: [fal, azure]
    default: fal
...
outputs:
  ...
  - path: <out>/assets/*.png (or *.svg)
    description: 16 real collage assets (model-generated by default, or the user's own real images).
  - path: <out>/nextjs/
...
example_prompt: |
  ...
  presets / 1 daily ritual. Generate original images for the visuals.
---
...
Pick the strategy from the **user's intent** — never default to a
placeholder. Real images only.

| Strategy          | When to choose                                          | Cost / latency        |
| :---------------- | :------------------------------------------------------ | :-------------------- |
| `generate`        | **Default.** Original visuals for any brief — hero, collages, scenes. Model-generated, embed the returned image. | ~$0.40, ~6 min        |
| `bring-your-own`  | User has their own art-direction images, **or** asks for a specific real subject (named brand / public figure / landmark) — drop the real images at `assets_path`. | $0, 0s                |
| `placeholder`     | **Deprecated — do not use for delivery.** SVG/CSS placeholders are banned by `craft/anti-ai-slop.md`. Only acceptable as a throwaway local wireframe the operator will overwrite before showing anyone. | $0, <1s               |

Set `inputs.imagery.strategy` accordingly. If unsure, use `generate`.

#### `generate` — gpt-image-2 mode (default, real images)
...
│   └── placeholder.ts       # DEPRECATED — SVG placeholder frames (never ship)
└── assets/
