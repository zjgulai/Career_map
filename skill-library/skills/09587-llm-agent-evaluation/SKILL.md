...
trigger: agent eval, 智能体评估, 评估策略, 基准测试, 对抗评估
description: LLM智能体评估策略
category: ai
version: 2.1.0
author: hermes-ai
...
> llm-agent-evaluation 是一个专业的 LLM智能体评估策略，帮助用户快速完成相关任务并输出标准化交付物。
...
```
agent eval, 智能体评估, 评估策略, 基准测试, 对抗评估
```
...
llm-agent-evaluation 属于 **开发运维** 域（development and DevOps for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 llm-agent-evaluation 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(llm-agent-evaluation)
    B --> C[分析结果]
    B --> D[解决方案]
```
