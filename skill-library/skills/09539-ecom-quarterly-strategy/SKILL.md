---
name: ecom-quarterly-strategy
trigger: ecom, quarterly, strategy
description: 电商季度战略分析工具，输入90天订单CSV，输出季度经营复盘、战略建议、年度对比、长期趋势洞察。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> ecom-quarterly-strategy 是一个专业的 电商季度战略分析工具，输入90天订单CSV，输出季度经营复盘、战略建议、年度对比、长期趋势洞察。，帮助用户快速完成相关任务并输出标准化交付物。
...
ecom-quarterly-strategy 属于 **电商运营** 域（e-commerce operations for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 ecom-quarterly-strategy 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(ecom-quarterly-strategy)
    B --> C[分析结果]
    B --> D[解决方案]
```
