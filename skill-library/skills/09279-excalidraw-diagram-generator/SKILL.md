...
> **本项目级安装路径**：`report02/.cursor/skills/excalidraw-diagram-generator/`（已随仓库同步，团队克隆后即可用）。  
> **本机用户级路径**（可选）：`~/.cursor/skills/excalidraw-diagram-generator/`（与 [awesome-copilot](https://github.com/github/awesome-copilot/tree/main/skills/excalidraw-diagram-generator) 同源）。  
> 在 **本仓库内** 运行脚本时，请将示例中的 `$HOME/.cursor/skills/excalidraw-diagram-generator/` 替换为：  
> `$(git rev-parse --show-toplevel)/.cursor/skills/excalidraw-diagram-generator/`（或在项目根目录使用相对路径 `.cursor/skills/excalidraw-diagram-generator/`）。  
> **辅助脚本**：需要 **Python 3.10+**（脚本使用 `X | Y` 类型注解）；若系统默认 `python3` 较旧，请用 `python3.12` / `python3.11` 运行 `scripts/*.py`。
...
3. **If library does NOT exist**: Respond with setup instructions:
   ...
   3. Create directory: $HOME/.cursor/skills/excalidraw-diagram-generator/libraries/[icon-set-name]/
   4. Place the downloaded file in that directory
   5. Run the splitter script:
      python $HOME/.cursor/skills/excalidraw-diagram-generator/scripts/split-excalidraw-library.py $HOME/.cursor/skills/excalidraw-diagram-generator/libraries/[icon-set-name]/
...
```bash
mkdir -p $HOME/.cursor/skills/excalidraw-diagram-generator/libraries/aws-architecture-icons
```
...
```bash
python $HOME/.cursor/skills/excalidraw-diagram-generator/scripts/split-excalidraw-library.py $HOME/.cursor/skills/excalidraw-diagram-generator/libraries/aws-architecture-icons/
```

**Step 5: Verify Setup**
After running the script, verify the following structure exists:
```
$HOME/.cursor/skills/excalidraw-diagram-generator/libraries/aws-architecture-icons/
...
2. **Add icons using Python script**:
   ```bash
   python $HOME/.cursor/skills/excalidraw-diagram-generator/scripts/add-icon-to-diagram.py \
...
3. **Add connecting arrows**:
   ```bash
   python $HOME/.cursor/skills/excalidraw-diagram-generator/scripts/add-arrow.py \
...
1. **Check for libraries**: 
   ```
   List directory: $HOME/.cursor/skills/excalidraw-diagram-generator/libraries/
