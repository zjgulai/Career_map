---
name: Anchor-Based Text Splitter
trigger: anchor, based, text, splitter
description: 基于锚点的精确文本分割工具，使用模糊匹配定位锚点位置，支持 OCR 错误容错。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> Anchor-Based Text Splitter 是一个专业的 基于锚点的精确文本分割工具，使用模糊匹配定位锚点位置，支持 OCR 错误容错。，帮助用户快速完成相关任务并输出标准化交付物。
...
Anchor-Based Text Splitter 属于 **通用** 域（general-purpose for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 Anchor-Based Text Splitter 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(Anchor-Based Text Splitter)
    B --> C[分析结果]
    B --> D[解决方案]
```
