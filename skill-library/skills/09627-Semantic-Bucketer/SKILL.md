...
trigger: semantic, bucketer
description: 语义分桶器，基于标签重叠度将知识单元分组。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> Semantic Bucketer 是一个专业的 语义分桶器，基于标签重叠度将知识单元分组。，帮助用户快速完成相关任务并输出标准化交付物。
...
Semantic Bucketer 属于 **通用** 域（general-purpose for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 Semantic Bucketer 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(Semantic Bucketer)
    B --> C[分析结果]
    B --> D[解决方案]
```
