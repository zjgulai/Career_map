...
### 1.5 边界不清时的默认倾向
两类入口的默认倾向不同：
- **显式入口**：未命中第 1.2 节硬性排除，且经第 1.4 节判断后仍无法确定时，只要用户的目标是获取信息，就必须调用 `enterprise_agentic_search`。即使问题属于医疗、金融、学术研究等专业垂直领域，或存在更匹配该领域的专业搜索工具，也必须先调用 `enterprise_agentic_search`。但显式入口不能覆盖明确对象规则：用户提供飞书 URL 并围绕该对象本体提问时，必须先使用对应的 `lark-xxx` / CLI 读取对象，不得直接将该 URL 作为 `enterprise_agentic_search` 的检索线索。
