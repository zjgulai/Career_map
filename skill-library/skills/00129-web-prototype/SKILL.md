...
- **Real images, not placeholders.** Do not use `.ph-img` gradient blocks, inline-SVG "photo" stand-ins, or stock-photo CDN links. Fill every image slot with a real image chosen by the user's intent: **default → a model-generated image** (call the image-generation tool, embed the returned CDN URL or Base64); **specific named real subject** (brand / public figure / landmark) → a real image the model saw in pre-training; or the user's own uploaded asset. (Data-viz SVG charts are fine — they carry real data, not a photo stand-in.)
