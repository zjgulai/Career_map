...
| 你在做的事 | 看哪节 | 一句话 |
|---|---|---|
| **创建任务**：发起一个新任务、跟踪进度、收结果、改派（在 shell / 别的 agent / 用户对话里发起） | [创建 / 派发任务](#创建--派发任务)、[跟踪 / 收结果](#跟踪--收结果) | 你显式传 task-id |
...
发起一个新任务、跟踪它、收结果、改派——在 shell / 别的 agent / 用户对话里发起。
...
aily-cli task create \
  ...
  --description-stdin <<'EOF'              # 必填；描述只从 stdin 读（唯一写法），verbatim 零转义
目标：汇总本周各组进展并产出报告
...
- **描述只有 `--description-stdin` 一种写法（`create` / `update` 同）**。规则：
  - 用 heredoc 时 **`--assignee` 等所有其它 flag 必须写在 `--description-stdin <<'EOF'` 之前**——`EOF` 之后的内容会被 shell 当成另一条命令、描述静默截断。
  - stdin 内容 verbatim 透传：多行 / Markdown / 代码块都不需要转义，别写 `"...\n..."` 这种字面量转义。
  - 空 stdin 两个命令都报错：`create` 描述必填非空；`update` 也必须非空——**描述不支持清空**（服务端要求 description 非空），只能改写成新内容。
...
aily-cli task update <task-id> --title "新标题"                             # 改标题
```

- `update` 可改 `--title / --description-stdin（描述从 stdin 读，须非空——不支持清空） / --assignee / --assignee-type (agent|agent_team) / --kind continuous / --interval / --next`，至少传一个。oneshot 升 continuous 走这里（见「周期任务」；反向 continuous→oneshot 服务端拒绝）。
...
```bash
aily-cli task create --parent "$AILY_CLI_TASK_ID" \
  --assignee XxxAgent --description-stdin <<'EOF'   # 可选 --assignee-type agent|agent_team
<子任务描述>
EOF
# ... 派多个 ...
exit 0
...
```bash
aily-cli task create --assignee BackendAgent \
  --kind continuous --interval 4h --description-stdin <<'EOF' # create 不收 --next，首轮按 interval 起
跟进到签约
EOF
...
- **想"等下一拍"时别提前收尾**（别用结果交付工具的 `status=done`）：那是"真的收尾"，会停掉调度（之后只能靠用户留评论 reopen）。只想等下次就别动状态，靠 `interval` 自动顺延。
