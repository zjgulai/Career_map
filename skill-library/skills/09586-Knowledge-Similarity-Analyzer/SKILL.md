...
trigger: knowledge, similarity, analyzer
description: 知识相似度分析器，从多维度计算知识单元（SKU）的相似度，识别重复、冲突、独立关系。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> Knowledge Similarity Analyzer 是一个专业的 知识相似度分析器，从多维度计算知识单元（SKU）的相似度，识别重复、冲突、独立关系。，帮助用户快速完成相关任务并输出标准化交付物。
...
Knowledge Similarity Analyzer 属于 **通用** 域（general-purpose for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 Knowledge Similarity Analyzer 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(Knowledge Similarity Analyzer)
    B --> C[分析结果]
    B --> D[解决方案]
```
