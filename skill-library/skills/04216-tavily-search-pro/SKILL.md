...
description: "Tavily AI search platform with 5 modes: Search (web/news/finance), Extract (URL content), Crawl (website crawling), Map (sitemap discovery), and Research (deep research with citations). Use for: web search with LLM answers, content extraction, site crawling, deep research. Tavily 深度搜索：AI 网页搜索/新闻/金融检索、URL 内容提取、网站爬取、站点地图与深度研究（带引用），需 TAVILY_API_KEY。"
version: 1.0.0
author: Leo 🦁
...
allowed-tools: [exec]
input_contract: 查询词或网址（需已配置搜索服务账号）
output_contract: 网页/新闻/金融搜索结果（可带摘要）、网址内容、站点地图或带引用的研究报告
example: 说「查一下最新 AI 新闻」→ 得到新闻结果列表（可带 AI 摘要）。
...
DSH 主进程由 GUI 启动，环境不读 ~/.zshrc，且 bash 每次调用都是新 shell——**每次调用脚本前先执行注入**：

```bash
export TAVILY_[REDACTED] -n 's/^export TAVILY_[REDACTED]"$/\1/p' ~/.zshrc | tail -1)
```
