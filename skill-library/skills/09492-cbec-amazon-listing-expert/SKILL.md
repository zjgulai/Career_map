---
name: cbec-amazon-listing-expert
trigger: cbec, amazon, listing, expert
description: 为 Amazon 母婴产品创建或重写 Listing，围绕购买心理、决策中断点和品牌优势提升转化。当用户提及"Amazon Listing"、"五点描述"、"主图"、"A+"、"EBC"、"转化优化"
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> cbec-amazon-listing-expert 是一个专业的 为 Amazon 母婴产品创建或重写 Listing，围绕购买心理、决策中断点和品牌优势提升转化。当用户提及"Amazon Listing"、"五点描述"、"主图"、"A+"、"EBC"、"转化优化"，帮助用户快速完成相关任务并输出标准化交付物。
...
cbec-amazon-listing-expert 属于 **跨境电商** 域（cross-border e-commerce for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 cbec-amazon-listing-expert 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(cbec-amazon-listing-expert)
    B --> C[分析结果]
    B --> D[解决方案]
```
