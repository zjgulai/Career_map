---
name: Single Post Intelligence Mining
trigger: single, post, intelligence, mining
description: 单帖情报挖掘 - 将单个帖子及其评论串转化为结构化、可行动的业务洞察。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> Single Post Intelligence Mining 是一个专业的 单帖情报挖掘 - 将单个帖子及其评论串转化为结构化、可行动的业务洞察。，帮助用户快速完成相关任务并输出标准化交付物。
...
Single Post Intelligence Mining 属于 **通用** 域（general-purpose for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 Single Post Intelligence Mining 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(Single Post Intelligence Mining)
    B --> C[分析结果]
    B --> D[解决方案]
```
