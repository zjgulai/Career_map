...
The skill generates a `.codebuddy_resources.json` file in the current working directory containing:
...
Optional arguments:
- `-o, --output <path>`: Specify custom output path (default: `.codebuddy_resources.json`)
- `-v, --verbose`: Print full resource information to stdout

### Step 2: Read and Apply Recommendations

After running detection, read the generated `.codebuddy_resources.json` file to inform computational decisions:

```python
# Example: Use recommendations in code
import json

with open('.codebuddy_resources.json', 'r') as f:
...
4. **Document decisions**: Keep the `.codebuddy_resources.json` file in project directories to document resource-aware decisions
