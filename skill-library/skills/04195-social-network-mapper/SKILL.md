...
title: "社交关系图谱分析"
description: "- Map social relationships between people using Twitter/X interactions (mentions, retweets, replies), and optional GitHub collaboration. Build and analyze social graphs, detect communities, and summarize clusters. Use when the user asks for \"谁和谁互动\", \"social graph\", \"Twitter 关系\", or when org-structure-research invokes it for relationship analysis."
triggers: 
  - social network
  - 互动
...
enabled: "true"
user-invocable: true
workflow: "解析种子名单并补全 Twitter 账号；抓取提及与回复构建互动边；用 NetworkX 构建关系图；社区检测并汇总聚类"
input_contract: 人物名单（姓名或社交账号）
output_contract: 互动关系图谱、社群聚类、核心人物榜与关系图
example: 说「看看这 20 个人谁和谁互动」→ 得到互动图谱和社群聚类。
...
## 触发词
- 社交关系图谱分析、social-network-mapper、基于社交平台互动构建关系图谱与社群聚类 等表述时使用。

## 何时不用
- 人物背景走 people-research；组织架构走 org-structure-research；单帖评论挖掘走 single-post-intel-mining
...
缺名单/关系数据时一次性追问：人员名单（含部门/角色）+ 互动渠道（邮件/会议/聊天）+ 时间范围；不全则先问。
