---
name: Lute Skills Creator
trigger: lute, skills, creator
description: `lute-skills-creator` 是一个 Skill 生产力工具，整合了：
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> Lute Skills Creator 是一个专业的 `lute-skills-creator` 是一个 Skill 生产力工具，整合了：，帮助用户快速完成相关任务并输出标准化交付物。
...
Lute Skills Creator 属于 **通用** 域（general-purpose for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 Lute Skills Creator 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(Lute Skills Creator)
    B --> C[分析结果]
    B --> D[解决方案]
```
