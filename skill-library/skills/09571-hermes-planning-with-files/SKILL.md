---
name: hermes-planning-with-files
trigger: planning, 文件化计划, markdown计划, 任务规划, 项目计划, 执行计划
description: 持久化Markdown计划
...
# hermes-planning-with-files

> hermes-planning-with-files 是一个专业的 持久化Markdown计划，帮助用户快速完成相关任务并输出标准化交付物。
...
```
planning, 文件化计划, markdown计划, 任务规划, 项目计划, 执行计划
```
...
hermes-planning-with-files 属于 **开发运维** 域（development and DevOps for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 hermes-planning-with-files 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(hermes-planning-with-files)
    B --> C[分析结果]
    B --> D[解决方案]
```
