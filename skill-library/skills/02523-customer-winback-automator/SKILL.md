...
title: "客户赢回自动化"
description: "- Identify and re-engage lapsed customers through automated multi-stage reactivation sequences. Implement tiered discounting based on historical LTV and establish sunsetting protocols for unengaged contacts. 边界：召回已流失客户；现有客户留存走 customer-retention-automator 触发词：客户召回、流失客户激活、复购唤醒、召回邮件序列、LTV 分层激励。"
...
whenToUse: "召回已流失客户；现有客户留存走 customer-retention-automator"
enabled: "true"
user-invocable: true
workflow: "识别流失客户并分层（45-90/91-180/181-365/365+ 天）；按 LTV 设计分层激励（高价值高激励、低价值低摩擦）；设计多阶段召回序列（重连、激励、最后通牒）；执行日落策略与列表卫生（再许可、抑制）"
input_contract: 流失客户分层信息（上次下单时间、历史消费金额）
output_contract: 分层召回方案：三步召回邮件序列+阶梯折扣规则+日落清理策略
example: 说「给90天没买的客户设计召回」→ 得到按45-90/91-180/181-365天分层的激励邮件序列
...
## 触发词
- 客户赢回自动化、customer-winback-automator、多阶段召回流失客户并分层折扣 等表述时使用。

## 何时使用
- 多阶段召回流失客户并分层折扣。

## 何时不用
- RFM 分群走 customer-rfm-analyzer；弃购挽回序列走 email-automation-flow-builder；忠诚度设计走 loyalty-points-designer
