...
Launch the implementer subagent using the `task` tool with `fork_context: true` so it inherits the full conversation history.
...
Launch a single reviewer subagent using the `task` tool with `fork_context: true`.
...
If `resume_from` fails (subagent expired), launch a fresh reviewer with `fork_context: true`, prepending the `reviewer` persona instructions to the prompt. Log a warning.
...
If `resume_from` fails for a reviewer (subagent expired), launch a fresh reviewer with `fork_context: true` for that specialization, prepending the persona instructions from `config.persona_to_inject` (if non-null) to the prompt. Log a warning.
...
- **fork_context=true on initial launches** — both the implementer and all reviewers get conversation history on their first launch so they understand the full context.
- **resume_from on follow-ups** — never launch a fresh subagent for fix or re-review rounds. Always resume the previous one so it retains its full working memory. Exception: if `resume_from` fails (subagent expired), launch a fresh one with `fork_context: true` and log a warning.
