...
title: "公司调研"
description: "当用户需要对一家具名公司做公司级调研、尽职调查或融资估值核查时使用：给出公司画像、融资历程、估值逻辑、团队治理、业务与竞争优势、风险与负面、近期动态，产出可追溯、可交叉验证、带 confidence 标注的研究报告。触发词：公司调研、尽职调查、融资估值、公司画像、投资者尽调、BD尽调。何时不用：产品选品（用 product-research-matrix）、市场进入 Go/No-Go（用 market-viability-logic-auditor）、Amazon 类目调研（用 amazon-sorftime-research）、SEO 竞品研究（用 seo-competitor-analysis）、常规竞品对比矩阵（用 competitor-profiling）、供应链供应商产能审查、招聘/个人背景调查、个人炒股决策。缺不可推定的关键材料（账号/文件/数值）才追问；可依行业惯例或品牌既定风格推定的，标注假设后继续，绝不编造数据。 Use when the user needs company-level research, due diligence, funding or valuation checks, investor/BD preparatio…"
user_summary: "给目标公司做深度尽调：融资、估值、团队、风险一次查清，输出可追溯的报告。要调研一家公司时，找它。"
user_try: "试试：「帮我调研一下这家公司的背景和风险」"
enabled: "true"
disable-model-invocation: true
user-invocable: true
workflow: "澄清研究对象、目标、时效与深度等级；生成多语言查询矩阵并分层搜索；按 source tier 筛选来源并提取事实；交叉验证并输出洞察链；自审后按输出模板交付"
---
...
- Amazon 类目数据分析，使用 `da-amazon-sorftime-research-research`
...
- **意图冲突**（company-research vs 选品／竞品对比）：先拆解明确优先级，再决定走本 Skill 还是相邻 Skill。
