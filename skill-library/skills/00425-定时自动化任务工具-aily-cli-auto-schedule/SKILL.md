...
| `list`            | 列出任务。`--trigger-type cron`/`interval` 返回**定时任务合集**（server 不分两者），`--trigger-type event` 只返 event，不传则全部一次列出 | 查看当前已有的任务 |
...
aily-cli auto create   创建自动化任务
                   ...
                   --interval N              间隔秒数，最小 300 (trigger-type=interval 时必填)
...
aily-cli auto list     列出所有任务
                   ...
                   --full                    返回完整 Auto 记录 (可选；默认 compact 5 字段)
...
aily-cli auto update   更新任务
                   ...
                   --interval N              新间隔秒数 (须配合 --schedule-desc)
...
7. **⚠️ 执行频率约束**：执行间隔（含 `--interval` 和 cron 等效间隔）不得低于 300 秒（5 分钟），**CLI 端直接 reject（exit 2）**。"实时通知"类需求建议转为定时批量检查。若后端另有频率限制返回拒绝，把原因转述给用户并请其确认调整方向，不要静默忽略。
...
17. **⚠️ 判定 CRUD 路径 — 操作前 list 一次**: 以**动词**判断意图，不以**名词**判断。任务名常含周期 / 行为词（如"**每日**飞书消息**总结**"），按动词识别（启用 / 停用 / 改 / 删…），别被名词带偏。
    ...
    - **list 输出优化**（按优先级）：
        ...
        2. 默认 compact 视图（5 字段）已够查重 + 数量检查
