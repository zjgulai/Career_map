...
export async function startEmbeddedMeeting(meetingNumber: string, userName: string, [REDACTED] {
...
ZoomMtg.join({
  ...
  // Authentication
  [REDACTED]              // Meeting password (capital W!)
  zak: string,                   // Host's ZAK token (required to start)
  tk: string,                    // Registration token (if required)
  obf[REDACTED]              // App Privilege Token (for 2026 requirement)
...
await client.join({
  ...
  // Authentication
  [REDACTED]              // Lowercase! (different from Client View)
...
document.getElementById('meeting_number').addEventListener('input', (e) => {
  ...
  const pwdMatch = e.target.value.match(/[REDACTED]
  if (pwdMatch) {
