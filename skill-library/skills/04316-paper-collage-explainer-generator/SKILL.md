...
description: Turn narration, opinions, or abstract ideas into paper-collage stills and stop-motion clips with tactile SFX and optional assembly. Not for presenter ads, editable layers, or prompt-only work.
trigger-words: [paper collage explainer, paper-collage animation, halftone collage, collage explainer, paper collage, stop-motion collage, collage animation]
---
...
The workflow prioritizes style continuity, color harmony, controlled paper texture, stop-motion collage rhythm, and an audio policy that keeps tactile collage SFX by default while explicitly not adding BGM, voiceover, or subtitles unless the user requests them.
...
- Default: do not add voiceover, spoken narration, or presenter narration. You may ask whether the user wants narration/voiceover when the project may benefit, but write, synthesize, or add spoken audio only after explicit user confirmation
...
1. During the first production-plan confirmation, state the default media approach explicitly: tactile paper-collage sound effects are kept or generated; BGM, voiceover/narration, and subtitles are not added by default.
2. It is acceptable to ask the user whether they want **voiceover narration**, **BGM**, or **subtitles**, especially for explainers, but present all three as optional add-ons, not defaults.
...
- Media approach, stated as: default collage SFX are kept or generated; BGM, voiceover/narration, and subtitles are not added unless the user explicitly requests them
- Optional add-on question when useful: ask whether the user wants voiceover narration, BGM, or subtitles, but keep them optional
...
- Use the approved still as the visual reference / final-frame anchor whenever the selected video model supports it
...
- Default: do not generate or add voiceover, spoken narration, or presenter audio unless the user explicitly requested narration/voiceover
...
Prefer MiniMax H3 because its native-audio capability fits the synchronized collage-SFX workflow. Honor another user-selected model when it meets the confirmed requirements. If it fails, is unavailable, or lacks a required capability, explain the difference and let the user choose an equivalent.
...
- Defaults: tactile collage SFX are desirable; BGM, voiceover/narration, and subtitles are absent unless explicitly requested
...
Add BGM, narration/voiceover, or subtitles only when explicitly requested. When the user requests BGM or narration after clips already exist, mix it with the collage SFX instead of replacing SFX, unless the user asks to mute the original SFX.
...
- Media note: collage SFX by default, and list any explicitly requested BGM, narration/voiceover, or subtitle additions
...
- If the production plan includes BGM, voiceover/narration, or subtitles without explicit user request, remove them and present BGM, voiceover/narration, and subtitles as optional choices.
