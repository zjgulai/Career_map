   ...
   - **Main grid (2/3 + 1/3)**:
     - Left: **Follower Growth · 30D** — a full-width inline SVG line
      ...
      - Right: **Top Post · This Week** — a card showing the rendered
        ...
        *Media block:* the bundled `example.html` ships a self-contained
        CSS/SVG demo thumbnail so the dashboard works offline and in
        screenshots. **For real delivery, replace it with a real image**
        (model-generated by default, or a real thumbnail of the actual
        post when the user provides one) — never ship the SVG demo block
        as the final visual. SVG remains correct for the charts/sparklines
        on this page (they carry real data).
   - **Lower grid (1/2 + 1/2)**:
