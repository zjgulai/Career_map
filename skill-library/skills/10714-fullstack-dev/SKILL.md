...
const config = {
  ...
  auth: { jwt[REDACTED]'JWT_SECRET'), expiresIn: process.env.JWT_EXPIRES_IN || '1h' },
} as const;
...
async function api<T>(path: string, options: RequestInit = {}): Promise<T> {
  const [REDACTED]  // from cookie / memory / context
...
async function apiWithRefresh<T>(path: string, options: RequestInit = {}): Promise<T> {
  try {
    ...
    if (err instanceof ApiError && err.status === 401) {
      const refreshed = await api<{ access[REDACTED] }>('/api/auth/refresh', {
        method: 'POST',
