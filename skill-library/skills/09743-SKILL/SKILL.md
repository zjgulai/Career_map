---
name: SKILL
trigger: skill, channel, operate/certification,
...
> SKILL 是一个专业的 当已有「目标市场合规清单」且可从 ERP/主数据读取现有认证与到期字段时调用；输出缺口排序与补证优先级。，帮助用户快速完成相关任务并输出标准化交付物。
...
```
skill, channel, operate/certification,
```
