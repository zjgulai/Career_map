...
title: "营销ROAS分析"
description: "- Track and analyze marketing spend across all channels with ROAS calculation, incrementality testing, and budget reallocation based on true profitability. 触发词：ROAS分析、广告花费分析、营销预算分配、渠道效率、增量测试。"
...
enabled: "true"
user-invocable: true
workflow: "计算盈亏平衡 ROAS；按渠道类型设定目标 ROAS；构建周度渠道绩效记分卡；基于真实盈利性再分配预算"
input_contract: 分渠道花费与收入数据；商品成本与变动费用率
output_contract: 盈亏平衡 ROAS+分渠道记分卡+预算加减与暂停决策建议
example: 说『帮我算各渠道 ROAS 怎么调预算』→ 得到记分卡与扩量/缩量决策
...
## 触发词
- 营销ROAS分析、marketing-roas-analyzer、全渠道投放与 ROAS 归因、预算再分配 等表述时使用。

## 何时不用
- 归因模型与增量实验走 attribution；广告账户审计走 paid-advertising；预算预测走 ecommerce-budget-forecaster
...
> 2026-09-07 SkillOpt b5 rollout：3 任务均分 80，轻量修复
