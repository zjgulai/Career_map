...
name: agent-browser
title: 浏览器自动化
description: "浏览器自动化操作：打开页面、点击/填表/勾选/悬停、抓取页面文本与属性、截图/PDF、等待元素、多会话隔离、Cookie 与状态保存、网络拦截。触发词：浏览器自动化、网页操作、打开网页、页面抓取、网页截图、点击填表、浏览器填表、网页文本提取。何时不用：需要人工视觉分析的复杂页面（用内置浏览器工具）。Headless browser automation CLI (accessibility tree snapshots + ref-based element selection)."
metadata: {"clawdbot":{"emoji":"🌐","requires":{"commands":["agent-browser"]},"homepage":"https://github.com/vercel-labs/agent-browser"}}
input_contract: 浏览器操作指令（打开页面/点击/填表/抓取文本；无需用户提供文件，需已安装 agent-browser CLI）
output_contract: 页面快照 JSON、元素文本/属性、截图或 PDF 文件，命令即时执行返回
