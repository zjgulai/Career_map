---
name: Lute Skills Opt
trigger: lute, skills, opt
description: `lute-skills-opt` 是一个用于优化 Skill 质量的工具 Skill。它读取 `lute-skills-eval` 生成的**结构化 YAML 评估报告**，通过**自动修复 + 人
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
> Lute Skills Opt 是一个专业的 `lute-skills-opt` 是一个用于优化 Skill 质量的工具 Skill。它读取 `lute-skills-eval` 生成的**结构化 YAML 评估报告**，通过**自动修复 + 人，帮助用户快速完成相关任务并输出标准化交付物。
...
Lute Skills Opt 属于 **通用** 域（general-purpose for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 Lute Skills Opt 帮我下一步行动或相关skill推荐...
```
