...
const { data: loginData, error: loginError } = await data.verifyOtp({ [REDACTED]' })
```
...
**3. Password**
```js
const { data, error } = await auth.signInWithPassword({ username: 'test_user', [REDACTED]' })
const { data, error } = await auth.signInWithPassword({ email: 'user@example.com', [REDACTED]' })
const { data, error } = await auth.signInWithPassword({ phone: '13800138000', [REDACTED]' })
```
...
await signUpData.verifyOtp({ [REDACTED]' })
```
...
const { data, error } = await data.verifyOtp({ email: "new@example.com", [REDACTED]" });

// Change password (logged in)
const { data, error } = await auth.resetPasswordForOld({ old_[REDACTED]', new_[REDACTED]' })

// Reset password (forgot)
const { data, error } = await auth.reauthenticate()
const { data, error } = await data.updateUser({ nonce: '123456', [REDACTED]' })
...
// Delete account
const { data, error } = await auth.deleteMe({ [REDACTED]' })
