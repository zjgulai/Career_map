...
trigger: agent pattern, 智能体设计模式, ReAct, Plan-and-Execute, 多代理架构
description: AI智能体架构模式库
category: ai
version: 2.1.0
author: hermes-ai
...
> agent-design-patterns 是一个专业的 AI智能体架构模式库，帮助用户快速完成相关任务并输出标准化交付物。
...
```
agent pattern, 智能体设计模式, ReAct, Plan-and-Execute, 多代理架构
```
...
agent-design-patterns 属于 **开发运维** 域（development and DevOps for Momcozy），可与其他 skill 形成工作流链路：

- **hermes-team**: 当本skill输出需要进一步处理时调用
- **llm-agent-evaluation**: 当本skill输出需要进一步处理时调用
- **error-handler**: 当本skill输出需要进一步处理时调用

## 一句话调用

```
使用 agent-design-patterns 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(agent-design-patterns)
    B --> C[hermes-team]
    B --> D[llm-agent-evaluation]
    B --> E[error-handler]
```
