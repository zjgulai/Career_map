...
title: "发票生成器"
description: "- Automate the generation of tax-compliant, branded PDF invoices for B2B and B2C orders, ensuring sequential numbering and legal data retention. 触发词：发票生成、PDF发票、自动开票、税务合规发票、增值税发票、账单自动化。"
...
enabled: "true"
user-invocable: true
workflow: "评估平台原生开票能力；选择实施模式（Stripe Invoicing 或无头 PDF 生成）；保证连续编号、税务字段与数据留存"
input_contract: 订单/开票平台与发票要求（公司信息、税号等）
output_contract: 合规PDF发票生成方案：实现步骤+税务必填项清单（连续编号、VAT等）
example: 说「给Shopify订单自动生成欧盟合规发票」→ 得到连续编号与税务字段齐全的开票方案
...
## 触发词
- 发票生成器、invoice-generator、生成税务合规的品牌 PDF 发票与顺序编号 等表述时使用。

## 何时不用
- 销售税/VAT 计算走 sales-tax-vat-automator；财务模型走 creating-financial-models；账期条款走 b2b-payment-terms-optimizer
