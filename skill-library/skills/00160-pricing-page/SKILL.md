...
1. **Read the active DESIGN.md** (injected above). Use only its colors, type
   ...
   - If Design Intelligence context is present, use its conversion pattern,
     CTA placement, UX checks, and anti-patterns to shape tier count and proof.
   - If Style Reference recommendations are present, use them as inspiration
     only; they do not override the active DESIGN.md.
...
3. **Sections**, in order:
   ...
   6. **Trust strip** when relevant — security, billing terms, cancellation,
      compliance, or payment-method reassurance. Keep it factual.
4. **Write** one self-contained HTML document:
   ...
   - `data-component` and `data-od-id` on each tier card and each table row.
   - Use real buttons for plan CTAs; links must not be `href="#"`.
   - Use `<table>` semantics for the comparison grid when possible. If using
     CSS grid, preserve row/column labels with accessible text.
...
6. **Self-check**:
   ...
   - The recommended tier has a reason, not only a badge.
   - Annual/monthly toggle has an accessible label and visible selected state.
   - FAQ answers address conversion objections: billing, cancellation, security,
     support, migration, and enterprise procurement.
   - No invented discounts or ROI claims unless supplied by the user.
7. **Lint repair loop**:
   - After writing, run the Canvas lint tool or inspect the write result's
     `repairInstructions`.
   - Fix all P0 and P1 items before handoff, especially empty links, labels,
     raw token drift, and responsive table behavior.
