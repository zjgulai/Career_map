...
export VAULT_[REDACTED]'

# Enable secrets engine
vault secrets enable -path=secret kv-v2

# Store secret
vault kv put secret/database/config username=admin [REDACTED]
```
...
deploy:
  ...
  script:
    - |
      DB_[REDACTED] kv get -field=password secret/database/config)
      [REDACTED] kv get -field=key secret/api/credentials)
...
- name: Get secret from AWS
  run: |
    [REDACTED] secretsmanager get-secret-value \
      --secret-id production/database/password \
      --query SecretString \
      --output text)
    echo "::add-mask::$SECRET"
    echo "DB_[REDACTED]" >> $GITHUB_ENV
...
def lambda_handler(event, context):
    ...
    current_[REDACTED]'SecretString'])
