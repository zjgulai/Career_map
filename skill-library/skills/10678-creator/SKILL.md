...
Kimi Code CLI loads skills from the configured skills directory when `--skills-dir` is provided.
Without that override, it checks user-level and project-level directories in priority order.
Daimon seeds its built-in skill assets into its managed skills root during onboarding, then starts
Kimi with that root as `--skills-dir`.
...
`--skills-dir` overrides user/project discovery and loads only that directory.
