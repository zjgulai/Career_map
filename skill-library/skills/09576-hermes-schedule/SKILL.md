---
name: hermes-schedule
trigger: schedule, cron, 定时任务, 定期执行, 监控, 定时检查, 自动化
description: 定时任务管理
...
> hermes-schedule 是一个专业的 定时任务管理，帮助用户快速完成相关任务并输出标准化交付物。
...
```
schedule, cron, 定时任务, 定期执行, 监控, 定时检查, 自动化
```
...
hermes-schedule 属于 **开发运维** 域（development and DevOps for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 hermes-schedule 帮我下一步行动或相关skill推荐...
```
