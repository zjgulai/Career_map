...
export class UserService {
  ...
  async createUser(userData: CreateUserDTO): Promise<User> {
    ...
    // Hash password
    const hashed[REDACTED] bcrypt.hash(userData.password, 10);
...
export class UserRepository {
  ...
  async create(userData: CreateUserDTO & { [REDACTED] }): Promise<UserEntity> {
    const query = `
...
export class AuthService {
  ...
  async login(email: string, [REDACTED] {
