---
name: SCM Main Hub - 供应链智能体主协调器
trigger: scm, main, hub, , , 供应链智能体主协调器
description: 中文版供应链智能体矩阵主协调器。整合库存预测专家、供应商评估专家、物流路线专家三个角色。
category: lute_cbec
version: 2.1.0
author: hermes-ai
...
# SCM Main Hub - 供应链智能体主协调器

> SCM Main Hub - 供应链智能体主协调器 是一个专业的 中文版供应链智能体矩阵主协调器。整合库存预测专家、供应商评估专家、物流路线专家三个角色。，帮助用户快速完成相关任务并输出标准化交付物。
...
SCM Main Hub - 供应链智能体主协调器 属于 **供应链** 域（supply chain management for Momcozy），可与其他 skill 形成工作流链路：
...
```
使用 SCM Main Hub - 供应链智能体主协调器 帮我下一步行动或相关skill推荐...
```
...
```mermaid
graph LR
    A[用户输入] -->|触发词| B(SCM Main Hub - 供应链智能体主协调器)
    B --> C[分析结果]
    B --> D[解决方案]
```
