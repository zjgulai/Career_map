---
name: SKILL
trigger: skill, channel, operate/listing, gate, d
...
> SKILL 是一个专业的 当需要把合规扫描结果转化为可执行上架决策（通过、补充资料、阻断）并输出可审计判定原因时调用。，帮助用户快速完成相关任务并输出标准化交付物。
...
```
skill, channel, operate/listing, gate, d
```
